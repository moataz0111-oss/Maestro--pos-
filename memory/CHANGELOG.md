...
### المشكلة (من المستخدم + صور، بيانات حقيقية لعميل)
منتجات الزنجر المرتبطة بمنتج مُصنّع "دجاج زنجر" (نفد مخزونه) تظهر في التقارير بتكلفة ضخمة (~143,000 للوحدة) وهامش ربح سالب −2500%، بينما حوار تعديل المنتج يعرض التكلفة الصحيحة (~1637).

### السبب الجذري (مؤكَّد بإعادة الإنتاج)
في `_enrich_unit_cost_fields` (`routes/inventory_system.py`): `denom = final_yield or stored_qty or 1.0`. عندما ينفد مخزون المنتج المُصنّع (quantity=0) ويتعذّر حساب العائد من الوصفة (yield=0)، يصبح `denom = 1.0` → تكلفة الوحدة = **تكلفة الدفعة كاملة** بدل (الدفعة ÷ العدد المُنتَج). كذلك استخدام المخزون الحالي يُضخّم التكلفة تدريجياً مع البيع.

### الإصلاح
- `_enrich_unit_cost_fields`: `denom = final_yield or total_produced or stored_qty or 1.0` (يعود للعدد الأصلي المُنتَج قبل المخزون الحالي/1).
- إضافة حقل `total_produced` لكل إسقاطات جلب المنتجات المُصنّعة التي تُغذّي حساب التكلفة: POS (3 مواضع في `server.py`)، والتقارير (`reports_routes.py` — `_resolve_product_unit_cost`).

### التحقق
- إعادة الإنتاج: قبل الإصلاح unit_cost=596,000 (= الدفعة)، بعده 1192 (= 596,000÷500). ✓
- مسار التقرير الفعلي `_resolve_product_unit_cost`: المنتج المرتبط أصبح 1192 بدل 596,000. ✓
- pytest جديد `tests/test_mfg_unit_cost_zero_stock_iter193.py` (3/3). لا تراجع (اختبارا inventory_stats الفاشلان قديمان قبل التغيير).

⚠️ يلزم **إعادة Deploy** لوصول الإصلاح لبيانات العميل الحقيقية.
