...
**السبب الجذري:** مصاريف بلا `shift_id` (أُنشئت قبل بدء الوردية) كانت تتسرّب لملخصها بسبب:
1. `shift_expense_query` كان يطابق مصاريف اليوم بالكاشير+الفرع بدون شرط "أُنشئت بعد بدء الوردية".
2. `create_expense` كان يترك `shift_id=None` إن لم توجد وردية مفتوحة → أي وردية لاحقة كانت تلتقط المصروف.

**التعديلات:**
- **`/app/backend/routes/shared.py`** `shift_expense_query`: إضافة شرط `created_at >= started_at` للـlegacy branch.
- **`/app/backend/server.py`** `create_expense`:
  - **Lazy Shift Opening للمصاريف**: لو الكاشير بدون وردية مفتوحة اليوم → تُفتح تلقائياً + المصروف يحصل `shift_id`.
  - المدير/المالك يبقى قادراً على إنشاء مصاريف عامة (بدون فتح وردية باسمه).
  - علامة `auto_opened_by_first_expense: True` لتتبّع.
- **`/app/backend/server.py`** post-expense cache: يستخدم `shift_expense_query` الصارم (بدل استعلام branch-only)

**اختبارات جديدة (4/4):**
- `/app/backend/tests/test_expense_shift_isolation.py`:
  - A: كاشير بدون وردية يُسجّل مصروفاً → وردية تُفتح تلقائياً.
  - B: بعد إغلاق وردية وفتح جديدة، مصاريف القديمة لا تظهر في الجديدة.
  - C: مصروف الأمس يبقى مع وردية الأمس؛ ملخص اليوم يظهر مصروف اليوم فقط.
  - D: عدة مصاريف في نفس الوردية → المجموع يطابق الملخص.

**النتيجة الإجمالية:** 10/10 اختبارات (287+288+290+291) نجحت 100%.
...
- **صفحة التواصل العامة `/contact`** (`SystemContact.js`) — يصلها الزبون عبر QR فاتورة الكاشير — أُعيد تصميمها بالكامل:
  ...
  - **أزرار أيقونات دائرية** (اتصال، واتساب، واتساب 2، بريد، الموقع) بدل الأشرطة الملونة — تظهر حسب توفر البيانات في الإعدادات.
  - السطر الجديد: «نظام محاسبي وإداري متكامل للمؤسسات والمطاعم والمشاريع التجارية الكبرى».
  - زر **«تعريف النظام»** يفتح نافذة برسالة المؤسس الكاملة (نفس نص الملف) + بطاقة المؤسس (Moataz Mehana / معتز مهنا).
  - **الأرقام بأرقام لاتينية** في كل اللغات (واتساب 2 بدل واتساب ٢).
  - **ترجمة كاملة** عربي/إنجليزي/كردي عبر `autoTranslate.js` (أُضيفت: بريد، الموقع، تعريف النظام، السطر التعريفي، و3 فقرات رسالة المؤسس) — وتم تمرير `promo_text`/`cta_text` عبر `t()`.
  - متجاوبة على الهاتف (الأيقونات بصف واحد عند 390px).
- **النصوص في قاعدة البيانات + النموذج:** حُدّث `promo_text` و`system_intro` في `db.settings` (type=system_invoice_settings) وافتراضيات `SystemInvoiceSettings` في server.py.
- **غلاف ملف الـ PDF (الصفحة 1)** في `scripts/generate_profile.py` أُعيد تصميمه (تصميم B): إطار ذهبي مزدوج + علامة مائية سداسية + شعار ذهبي موسّط + بطاقة اسم بإطار ذهبي + السطر الجديد. أُصلح انزياح الفاصل الذهبي (inline-block بدل margin:auto). الناتج: `/app/frontend/public/Maestro-EGP-Profile.pdf` (7 صفحات).
- SW cache bumped: sw-offline v17, sw-customer v7, sw-driver v5.
- **بيئة:** أُعيد تثبيت cairosvg/weasyprint وتشغيل mongod (تساقط متكرر). فشل اختبار الأيقونات كان بسبب توقف mongod فقط — لا خطأ برمجي؛ بعد إعادة التشغيل تظهر الأيقونات (4 أزرار) بشكل صحيح.


## Session: 16 Jun 2026 (fork) — تحسين OCR الفواتير + فصل شفتات البصمة ✅
- **(P0) زر «تحليل الفاتورة» (OCR) أصبح سريعاً ودقيقاً وموثوقاً:** أُعيدت كتابة `POST /api/purchase-invoices/ocr` (server.py):
  - **ضغط/تصغير الصورة** قبل الإرسال عبر Pillow (`_resize_invoice_image_b64`: تصحيح دوران EXIF، تحويل RGB، أقصى ضلع 1600px، JPEG جودة 80) → يحل فشل صور الموبايل الكبيرة.
  - **موديلان رؤية مع fallback:** `gemini-3-flash-preview` (أساسي) ثم `gpt-5.4-mini` (احتياطي) عبر emergentintegrations (EMERGENT_LLM_KEY).
  - **استخلاص JSON متين** (`_extract_json_from_text`: بلوك markdown، ثم محاولة مباشرة، ثم أول كائن `{}` متوازن) + إعادة محاولة على الموديل التالي عند فشل القراءة.
  - الاستجابة: `{success, data, model}` (متوافقة مع الواجهة الحالية PurchasesPage/Purchasing بلا تغيير frontend).
  - تحقق: curl حيّ عبر الرابط الخارجي — صورة صغيرة وصورة موبايل 3000×4000 كلتاهما استُخرجتا بنجاح (invoice_number/supplier/items/total) بموديل gemini-3-flash-preview.
- **(P1) فصل الشفتات في البصمة (لا مزيد من شفت 24 ساعة):** أُعيدت معالجة البصمات في `_auto_process_attendance_internal` (server.py) لتكون **زمنية صحيحة**:
  - تُخزَّن البصمة بتاريخها التقويمي الحقيقي وتُرتَّب **زمنياً** (بـ datetime كامل) بدل الترتيب النصي → يصلح ترتيب الشفت الليلي (22:00→00:09 بدل 00:09→22:00).
  - لو امتدّت بصمات يوم العمل أكثر من 16 ساعة وبفجوة ≥ 6 ساعات → يُعتبر شفتين مدموجين، فتُرحَّل المجموعة المبكرة (انصراف الليلة السابقة، مثل 00:09) إلى يوم العمل السابق، ويبقى حضور الشفت الجديد (مثل 23:57) لوحده.
  - `_order_shift_times` (دوران عند أكبر فجوة) + `_gap_after` (فرق دقائق مع دوران منتصف الليل) في توزيع البصمات ومسار الدمج مع سجل سابق.
  - تحقق: `tests/test_biometric_shift_separation.py` (2/2: شفت ليلي + نهاري) + تأكيد عدم انحدار اليوم النهاري العادي 4-بصمات (09:02→17:05 −استراحة = 7.55س).
- **بيئة:** أُعيد تثبيت الحزم المتساقطة (uvicorn/fastapi/weasyprint/cairosvg/...) وتشغيل mongod؛ أُضيف `weasyprint==69.0` و`CairoSVG==2.9.0` إلى requirements.txt (كانا مفقودين فيتساقطان).



## Session: 16 Jun 2026 (تابع) — إصلاح ساعات الحضور + رتوش الأذونات ✅
- **إصلاح حساب «الساعات» في الحضور:** كان يستخدم `.seconds` (مكوّن الثواني فقط) بلا معالجة للورديات الليلية → 0.0/23.8 خاطئة. أُضيف helper `_calc_worked_hours_hhmm` (total + وردية ليلية +1440 دقيقة + خصم الاستراحة) في server.py (create/update attendance، biometric sync، daily report). والواجهة (HR.js) تُعيد حساب العمود دائماً من البصمات بدل الثقة بقيمة مخزّنة خاطئة. تحقق: 00:59→13:00=12.02، 15:52→03:00=11.13، مع استراحة=7.0.
- **إجازة السفر/السنوية تُدفع من خزينة المالك:** عند الاعتماد يُنشأ سحب (owner_withdrawals، category=payment) باسم الموظف بقيمة days×daily_rate ويُخصم من رصيد فرعه، وتُسجَّل أيامها status=annual_leave (لا دفع مزدوج). `EmployeeResponse` يكشف annual_leave_balance + عرض الرصيد المتبقي في النموذج.
- **رتوش:** زر «شهر كامل» و«أسبوع» سريع في نموذج الإجازة + زر «طباعة» لتقرير الأرشيف (نافذة طباعة HTML). رفع كاش SW (offline v16).


## Session: 16 Jun 2026 — ميزة أذونات وإجازات الموظفين (موافقة المالك) ✅

**الميزة:** ٣ أزرار في تبويب «الحضور» بالموارد البشرية + تبويب أرشيف «الأذونات والإجازات»:
- 🤒 **عطلة مرضية** (أيام): تُسجَّل حضوراً مدفوعاً ضمن راتب الشهر بلا خصم.
- ⏱️ **إذن ساعات**: تُحتسب الساعات كعمل (مكافأة معادِلة type=permission) بلا خصم.
- ✈️ **إجازة سفر/سنوية**: تُخصم من رصيد الإجازة السنوية + **راتب الإجازة كامل يُسحب من خزينة المالك باسم الموظف ويُخصم من رصيد فرعه** (owner_withdrawals، category=payment، تُحتسب status=annual_leave فلا دفع مزدوج بالراتب الشهري).

**سير العمل:** المدير/مدير الفرع يُدخل ويوقّع → status=pending → **شارة إشعار للمالك** (pending-count) → المالك (admin/super_admin) يوافق/يرفض → عند الموافقة تُحتسب للموظف. أرشيف كامل يوضّح من منح ومن اعتمد.

**Backend:** `routes/payroll_routes.py` — endpoints `/api/leave-permissions` (POST create, GET list, GET pending-count, PUT {id}/approve, PUT {id}/reject). `EmployeeResponse` يكشف الآن `annual_leave_balance`.
**Frontend:** `pages/HR.js` — تبويب leaves + 3 أزرار + نموذج + جدول أرشيف + شارة معلّقة + عرض مبلغ راتب الإجازة.
**Tests:** `backend/tests/test_leave_permissions.py` (+ testing agent iter242: 7/7 backend، الواجهة 100%). تحقق منفصل: سحب إجازة السفر خصم من المحفظة بالضبط (−50,000). رفع كاش SW (offline v15).



## Session: 15 Jun 2026 (fork iter241) — إصلاح ملف PDF + تقارير المشتريات/زيادة الأسعار + تطبيق السائق ✅

**أ) ملف PDF التعريفي (Maestro EGP):** السبب الجذري لتغيّر الخط = خط Cairo كان يسقط من البيئة فيستخدم WeasyPrint خطاً بديلاً. الحل: تضمين الخط عبر `@font-face` يشير لملف المستودع في `generate_profile.py` (يبقى دائماً)، وإضافة توهج للشعار. الناتج: `/Maestro-EGP-Profile.pdf` (≈173KB، الخط مضمّن). نفس الألوان والتخطيط والنص مع التعديلات الأربعة فقط.

**ب) صورة الفاتورة (تقرير المشتريات الخارجية):** كانت تُحفظ بمسار `/uploads/` (يُوجَّه للواجهة عبر ingress فلا تظهر). الآن `/api/uploads/invoices/` + تطبيع `imgUrl()` للروابط القديمة + ترحيل DB. مُختبر HTTP 200 image/png.

**ج) تقرير زيادة الأسعار:** `POST /api/purchases-new` يحسب الآن `price_increase_log` + `tenant_id` + `created_by` ويفرض تبرير الزيادة ≥+10% (400 code=PRICE_INCREASE_REASON_REQUIRED). الواجهة ترسل `price_increase_reasons`. التقرير يعرض البيانات.

**د) الإجراءات السريعة:** بطاقة external-purchases-report باسم ديناميكي ("تقرير المشتريات الخارجية" مركزي / "مشتريات ومخزن الفرع" لكل فرع) وتظهر في الوضعين.

**هـ) تطبيق السائق:** بعد "بدء" التتبع يُخفى الزر ويُستبدل بشارة "نشط" خضراء (لا إيقاف إلا من إعدادات الهاتف). الطلب المرفوض يُحرَّر من السائق (مسح driver_id/name/phone في reject_order و reject_customer_order).

**و) إصلاحات إضافية:** GET /api/purchases-new كان 500 (حقول قديمة ناقصة) → جُعلت Optional. تنبيهات الإدارة محدودة بـ3 + pointer-events-none لمنع حجب النقر. رفع كاش SW (offline v12, customer v6, driver v4). السجل الأمني موجود فقط في لوحة SuperAdmin (تم التحقق).



## Session: 15 Jun 2026 (fork iter66) — تقرير المشتريات الخارجية + سداد الموردين من الخزينة + إصلاحات عرض + طباعة A4 + Push للمكالمات + إيصال مخالصة ✅

**أ) إشعار Web Push للمكالمة الواردة:** عند `POST /api/calls/initiate` يُرسَل Push للطرف المستلِم (سائق/زبون) ليرنّ هاتفه بالخلفية. تحديث notificationclick في sw-push.js/sw-driver.js ليُركّز النافذة بدل التوجيه عند نوع `incoming_call`.

**ب) إيصال مخالصة نهائية للموظف (A4):** `printSettlementReceipt` في HR.js (يُطبع عند الصرف + زر إعادة طباعة للمؤرشفين) + endpoint `GET /api/employees/{id}/settlement-receipt` (إعادة احتساب التفاصيل).

**ج) إصلاح باغ النصوص الحرفية:** كان `PurchasesPage.js` يعرض `{purchase.purchase_number}` كنص حرفي (تعابير JSX مغلّفة داخل `t('...')`). أُصلح + ١٠ مواضع مشابهة (Invoices, Loyalty, Recipes, Settings, WarehouseTransfers, Coupons).

**د) ⚠️ سبب رؤية المستخدم للباغ القديم:** عامل الخدمة `sw-offline.js` (Cache-First للـ JS). رُفعت نسخ الكاش: sw-offline v7→v8، sw-customer v3→v4، sw-driver v1→v2 (تُمسح الحزم القديمة تلقائياً). الكود في المعاينة سليم؛ يلزم **إعادة فتح التطبيق** و**النشر (Deploy)** ليصل الإصلاح للإنتاج.

**هـ) تقرير المشتريات الخارجية (ميزة كبيرة جديدة):**
- صفحة مستقلة `ExternalPurchasesReport.js` على `/external-purchases-report` — تظهر في الإجراءات السريعة + أزرار المصاريف اليومية **فقط في وضع `per_branch`** (تُخفى في `centralized`).
- Backend (`inventory_system.py`): `GET /api/purchases-report` (الدائن/المدين، فواتير، تجميع الموردين، رصيد الخزينة الكلي)، `POST /api/purchases-new/{id}/pay` (سداد كامل/جزئي يُخصم من **إجمالي رصيد الخزينة** عبر owner_withdrawals category=`purchase_payment`)، `DELETE .../payments/{id}` (إلغاء دفعة وإعادة المبلغ).
- الواجهة: بطاقات إجمالية، تبويب الفواتير (قابلة للضغط → تفاصيل: مواد، صورة، مسؤول المشتريات، سجل دفعات، تسديد)، تبويب الموردين (إجمالي/مسدّد/متبقي/كميات).
- **طباعة A4 فقط** (لا PDF/Excel حسب طلب المستخدم): `printInvoiceA4` (فاتورة مفردة) + `printSupplierStatementA4` (كشف حساب مورد).
- أزرار في المصاريف اليومية (`Expenses.js`) تنتقل لصفحة المشتريات وتفتح حوار «إضافة مورد»/«إنشاء فاتورة» تلقائياً (عبر location.state في PurchasesPage).

**اختبارات:** `tests/test_purchases_report_iter66.py` (سداد جزئي/كامل/رفض التجاوز + خصم الخزينة) — 5/5 تمر مع iter65.
**ملاحظة مستقبلية (مؤجّلة بطلب المستخدم):** توسعة النظام لـ«نظام مؤسسة» شامل لكل المشاريع (لا مطاعم فقط) — يُدرَس ويُناقَش لاحقاً قبل التنفيذ.



## Session: 14 Jun 2026 (fork iter65) — إصلاح اختفاء طلب السائق + مكالمة صوتية داخل التطبيق (WebRTC) + أذونات الميكروفون ✅

**0) إصلاح بيئة:** الباك إند كان متوقفاً (FATAL) بسبب فقدان `uvicorn`/`bcrypt` + توقّف `mongod`. أُعيد تثبيت الحزم وتشغيل mongod. (مشكلة متكررة معروفة.)

**1) 🔴 باغ: الطلب يختفي من تطبيق السائق بعد ~دقيقة — مُصلَّح ومُتحقَّق ✅**
- السبب الجذري: `GET /api/driver/orders` (server.py ~21060) كان يحصر الحالات بقائمة بيضاء `[pending, preparing, ready, out_for_delivery]`. عند تغيّر حالة الطلب إلى `confirmed` (نقطة `/notifications/accept-order`) أو `completed` (تأكيد POS/الدفع) كان الطلب **يختفي من السائق قبل التسليم**.
- الحل: أصبح يُرجع كل الطلبات المسندة للسائق **ما عدا** الحالات النهائية `[delivered, cancelled, canceled, refunded, rejected]`. + إضافة status_labels لـ confirmed/completed. + الواجهة (`DriverApp.js`): `activeOrders`/`getStatusBadge` تدعمان الحالات الجديدة.
- تحقّق: بقاء الطلب بحالة confirmed ظاهراً للسائق (curl + بصري) + pytest.

**2) 🟠 مكالمة صوتية حقيقية داخل التطبيق (زبون ↔ سائق) — WebRTC + STUN ✅**
- الإشارات (signaling) عبر HTTP polling مع تجميع ICE كامل (non-trickle) — لأن Socket.IO غير مُقدَّم فعلياً (uvicorn يشغّل `server:app` لا `socket_app`).
- Backend جديد `routes/call_routes.py` (عام بلا JWT): `POST /api/calls/initiate`، `GET /api/calls/incoming?driver_id|order_id|phone`، `GET /api/calls/{id}`، `POST /api/calls/{id}/answer|reject|end`. مجموعة `call_sessions` (رنين 45ث TTL).
- Frontend: hook موحّد `hooks/useWebRTCCall.js` (STUN عام: Google) + مكوّن `components/CallUI.jsx` (شاشة مكالمة كاملة: واردة بقبول/رفض ورنين، صادرة، متصل مع كتم/إنهاء ومؤقّت). مدمج في `DriverApp.js` (زر "اتصال" بكل طلب) و`CustomerMenu.js` (خيار "اتصال داخل التطبيق" في bottom-sheet التواصل).
- تحقّق: دورة الإشارات الكاملة (initiate→incoming→answer→status→end) عبر curl + pytest، وواجهة المكالمة الواردة تظهر بصرياً للسائق (قبول/رفض/اسم المتصل).
- ⚠️ STUN فقط — قد تفشل المكالمة على شبكات NAT متماثل بدون خادم TURN (مؤجّل حسب موافقة المستخدم).

**3) 🟡 أذونات الميكروفون — ✅**
- يُطلب الميكروفون عبر `getUserMedia` عند بدء/قبول المكالمة + تهيئة مسبقة `primeMic()` عند دخول السائق وفتح ورقة تواصل الزبون. (إذن الإشعارات مطلوب أصلاً، الموقع مطلوب للسائق.)

- اختبارات: `tests/test_calls_and_driver_orders_iter65.py` (3/3 تمر).



## Session: 12 Jun 2026 — تخطيط POS على الهاتف + لغة تطبيق الزبائن (من تعليقات صورتين)

**المشكلة ١ (POS على الهاتف) — مُصلَّحة ومُتحقَّقة ✅**
- الشكوى: الفئات لا تقبل التمرير، زر "الطلبات المعلّقة" مقصوص، زر الرجوع لا يظهر، الشاشة يجب أن تَسَع كل شيء.
- السبب الجذري: المجموعة اليسرى في رأس الصفحة (`flex items-center gap-2`) بلا `flex-wrap` فتبقى عناصرها على سطر واحد يفيض ويُقَص على الهاتف؛ وقائمة الفئات ملفوفة بـ Radix `<ScrollArea>` التي تمنع التمرير الأفقي باللمس.
- الإصلاح (`pages/POS.js`): (أ) أُضيف `flex-wrap` للمجموعة اليسرى في الهيدر فيظهر كل شيء (الرجوع، العنوان، اختيار الفرع، الطلبات المعلّقة). (ب) استُبدلت `<ScrollArea>` حول الفئات بـ`<div className="flex-1 overflow-auto">` فصار التمرير الأفقي للفئات يعمل على الهاتف ويبقى العمودي على الديسктоп.
- تحقّق بصري (390px): زر الطلبات المعلّقة **ظاهر بالكامل**، الرجوع ظاهر، الفئات تُمرَّر، `body.scrollWidth=390` (لا فيض أفقي).

**المشكلة ٢ (لغة تطبيق الزبائن) — توضيح + إصلاح جزئي ✅**
- الشكوى: عند اختيار الإنجليزية تُعاد الصفحة ولا تترجم.
- التشخيص (تحقّق حيّ): نظام الترجمة **يعمل فعلاً** — `changeLanguage('en')` يحفظ `app_language` ويُعيد التحميل، ونصوص الواجهة الملفوفة بـ`t()` تُترجم ("Search for product"، "All"، واتجاه LTR). **النصوص التي تبقى عربية هي بيانات** (أسماء الفئات والمنتجات وعملة "د.ع" واسم المطعم) أدخلها المستخدم في القائمة، ولا يمكن لـ`t()` ترجمتها — ليست خطأ ترجمة.
- إصلاح: لُفّ العنوان المكتوب يدوياً `الفروع المتاحة` بـ`t()` وأُضيفت ترجمته في `autoTranslate.js` ('Available Branches').
- **متبقٍّ (ميزة، ليست خطأ)**: لعرض أسماء الفئات/المنتجات بالإنجليزية يلزم حقول ثنائية اللغة (name_en) لكل منتج/فئة + واجهة إدخالها — ميزة منفصلة.

---



## Session: 11 Jun 2026 (تكملة 14) — إصلاح 502 في النشر + أسماء الفروع "غير محدد" في سجل التحصيلات

- **(P0) سبب الـ502 في النشر (Production) — مُصلَّح ✅**: الوحدات التي استخرجتها (break_even / reservations_reviews / suppliers) كانت تنشئ عميل MongoDB عبر `db = get_database()` **على مستوى الموديول (وقت الاستيراد)**. هذا يعمل في المعاينة (`uvicorn --reload`) لكنه يربط العميل بحلقة أحداث (event loop) خاطئة/مغلقة على خادم الإنتاج → تعطّل الإقلاع → 502. الحل: استبدال الاستدعاء بمُوكِّل كسول `_LazyDB` يحلّ العميل **وقت الطلب** (الحلقة الصحيحة)، مطابقاً نمط `reports_routes`/`drivers_routes` (التي تستدعي `get_database()` داخل كل دالة). تحقّق: استيراد الوحدات لا ينشئ عميلاً وقت الاستيراد، كل النقاط 200، 14/14 اختبار.
- أُضيف `CORS_ORIGINS=*` إلى `backend/.env` (كان يفترض `*` افتراضياً؛ أُضيف لتمرير فحص النشر).
- **ملاحظة عن فحص النشر**: أداة فحص النشر تُعلّم تبعيات الذكاء (openai/litellm/tiktoken/google-genai) كـ"حاجبات" لكنها **موجودة مسبقاً** ومطلوبة لميزات OCR/الذكاء في التطبيق، والتطبيق كان يُنشر بها سابقاً — ليست سبب هذا الـ502، ولا يجب حذفها (ستكسر ميزات قراءة الفواتير). تحذير "supervisor مفقود" يوفّره النظام وقت النشر.
- **(P1) أسماء الفروع تظهر "غير محدد" في سجل التحصيلات — مُصلَّح ✅**: السجلات القديمة في `delivery_collections` خُزّنت بـ`branch_name = null` داخل `branch_breakdown` (مع وجود `branch_id`). أُضيف في `GET /reports/delivery/collections` حلّ اسم الفرع من `branch_id` عبر خريطة `branches` (id→name) للسجل الأعلى ولكل عنصر تفصيل. تحقّق: عنصر بـ`branch_name=null` صار يعرض "فرع الجادرية".

⚠️ يجب **Save to GitHub + Deploy** لتطبيق إصلاح الـ502 وأسماء الفروع على maestroegp.com.

---



## Session: 11 Jun 2026 (تكملة 13) — متابعة تقسيم server.py (وحدتان إضافيتان)

- استُخرجت مجموعتان نظيفتان جديدتان من `server.py` (مع نقل نماذج Pydantic الخاصة بها):
  - `routes/reservations_reviews_routes.py` — الحجوزات + التقييمات (9 نقاط: reservations CRUD+stats، reviews CRUD+respond+stats).
  - `routes/suppliers_routes.py` — الموردون (4 نقاط CRUD؛ نماذج Supplier نُسخت داخلياً مع إبقاء الأصل لأن purchase-orders يعتمد عليها).
- إجمالي ما استُخرج هذه الجلسة: 4 وحدات (sales_target, break_even, reservations_reviews, suppliers). `server.py`: 24205 → **22947 سطر** (~1258 سطر أقل).
- تحقّق: كل النقاط المستخرجة 200 + الجيران (purchase-orders, smart-reports, orders, products) سليمة. اختبار الانحدار `tests/test_refactor_extracted_routes.py` **14/14 ✓**.
- **المتبقّي (P2)**: لا يزال ~400 نقطة في server.py. الاستمرار وحدة-وحدة آمن (مجموعات تالية مرشّحة: login-backgrounds, printer-types, ratings, promotions/coupons/loyalty...). وكذلك تقسيم `Settings.js`.

---



## Session: 11 Jun 2026 (تكملة 12) — توحيد الخريطة (OSRM في إدارة التوصيل) + توضيح حالة الطلبات المعلّقة/المرفوضة

**رد على طلب المستخدم (3 مهام يظن أنها مكتملة):**

1. **(خرائط موحّدة) — أُكمل الجزء الناقص ✅**: تطبيق الزبائن/السائق/التتبّع العام كانوا يستخدمون OSM + مسار طرق OSRM، لكن **خريطة إدارة التوصيل** (`components/DriverTrackingMap.js`) كانت ترسم **خطاً مستقيماً متقطعاً** فقط بين السائق والوجهة. أُضيف جلب مسار الطرق الحقيقي من OSRM (`router.project-osrm.org`) واستبداله بالخط المستقيم (مع إبقاء المستقيم كاحتياط لو فشل OSRM) — نفس آلية خريطة الزبائن تماماً. البلاطات (tiles) كانت موحّدة أصلاً (OpenStreetMap). الآن الخرائط الأربع موحّدة.

2. **(الطلبات المعلّقة/المرفوضة) — مُطبّقة فعلاً في الكود (تحقّق):**
   - بقاء طلب التوصيل معلّقاً حتى التحصيل: `POS.js` (أسطر ~900‑911) — الطلب المُسلَّم (delivered) يبقى في "المعلّقة" ما دام `driver_payment_status !== 'paid'` و`payment_status` ليس paid/credit. لا يختفي بضغط السائق "تم الاستلام".
   - الطلبات المرفوضة لا تظهر في إدارة الطلبات: `GET /orders` (server.py ~6944) يستبعد `is_rejected=True` افتراضياً، وتظهر فقط في إدارة التوصيل عبر `include_rejected`.
   - ⚠️ إن ظهر السلوك القديم على maestroegp.com فالسبب أن الكود لم يُنشر بعد (Save to GitHub + Deploy)، أو حالة محددة تحتاج خطوات إعادة إنتاج.

3. **(تقسيم server.py وSettings.js) — مستمر تدريجياً**: استُخرجت سابقاً `sales_target_routes.py` و`break_even_routes.py`. الباقي يُنفَّذ وحدة-وحدة مع اختبار بعد كل خطوة (خطر انحدار على نظام إنتاجي).

---



## Session: 11 Jun 2026 (تكملة 11) — تطبيق الزبائن: حذف الأقسام الداخلية من اختيار الفرع + أسماء فروع فسفورية

**طلب المستخدم (تعليق على صورة):** "تطبيق الزبائن يظهر فيه الأقسام مع الفروع، يجب أن تظهر الأقسام فقط في الموارد البشرية — احذفهم من التطبيق (المطبخ المركزي والمخزن وقسم المشتريات). والخطوط لأسماء الفروع غير واضحة، خليها نفس اللون لكن فسفوري فاتح."

- **(P0) حذف الأقسام الداخلية من شاشة اختيار الفرع للزبون — ✅**: السبب الجذري أن `GET /customer/menu/{tenant_id}` (server.py) كان يُرجع **كل** الفروع النشطة بلا فلترة، بينما الأقسام الداخلية مُخزّنة كـbranches بـ`branch_type` ضمن `["central_kitchen","warehouse","purchasing"]` (تقارير المبيعات/التعادل تستبعدها أصلاً). الإصلاح: أُضيف `branch_type: {$nin: [...]}` لاستعلام فروع الزبون. تحقّق حيّ: بعد إضافة 5 فروع تجريبية، التطبيق يُرجع 3 فروع حقيقية فقط ويستبعد الـ3 أقسام. (الأقسام تبقى ظاهرة في الموارد البشرية وبقية النظام — السلوك الصحيح.)
- **(P1) وضوح أسماء الفروع (فسفوري) — ✅**: في `CustomerMenu.js` (شاشة `branches`)، كانت أسماء الفروع `text-gray-800` (رمادي غامق) على بطاقات داكنة فتختفي. الإصلاح: البطاقة أصبحت داكنة بثبات (`bg-neutral-900/95` + حدّ برتقالي)، واسم الفرع برتقالي فسفوري مضيء `text-orange-400` مع توهّج `[text-shadow:_0_0_10px_rgb(251_146_60_/_70%)]`، والعنوان/الهاتف `text-orange-200/90`. تحقّق مرئي عبر screenshot: الأسماء واضحة ومضيئة على iPhone (430px).

---



## Session: 11 Jun 2026 (تكملة 10) — تحقّق ماسح الباركود + إصلاح تجاوز Dashboard + بدء تقسيم server.py

- **بيئة العمل (Fork)**: عند الإقلاع كانت حزم Python مفقودة (uvicorn...) وخدمة MongoDB متوقفة. أُعيد تثبيت `requirements.txt` وشُغّلت `mongod` عبر `/etc/mongod.conf`. الخادم يعمل وتسجيل الدخول 200. (ملاحظة للوكيل القادم: إن فشل الباك إند، `pip install -r requirements.txt` + تشغيل mongod.)
- **(P0) تحقّق ماسح الباركود في POS — ✅**: في POS → نوع الطلب "توصيل" → اختيار شركة توصيل (مثل توترز) يظهر زر المسح (`scan-company-order-id-btn`) بجانب حقل `delivery-company-order-id`، والنقر يفتح نافذة `barcode-scanner-dialog` (عنوان "مسح باركود الطلب"). رسالة "تعذّر تشغيل الكاميرا" متوقّعة في متصفح headless بلا كاميرا/إذن — النافذة تُرندر وتُعالج الخطأ بأمان بلا أي كراش. على جهاز/PWA حقيقي ستمسح فعلياً.
- **(P2) تجاوز عرض Dashboard على الموبايل — ✅**: الصفحة لم تعد تُمرَّر أفقياً (body.scrollWidth = عرض الشاشة 390). كان متبقٍّ تجاوز 6px من حشوة الهيدر `px-6` على الموبايل → غُيّرت إلى `px-3 sm:px-6`.
- **(P2) بدء تقسيم server.py (24k سطر) — مرحلة 1**: أُنشئ نمط استخراج آمن تدريجي معتمد على `routes/shared.py`.
  - `routes/sales_target_routes.py` (نقطتا `/sales-target` GET/POST).
  - `routes/break_even_routes.py` (4 نقاط `/break-even/*`: daily / daily-range / monthly-summary / alerts).
  - حُذف ما يقارب 970 سطراً من server.py (24205 → 23232). كل النقاط المستخرجة تُرجع 200 + النقاط المجاورة (smart-reports/sales, reports/sales) سليمة.
  - اختبار انحدار جديد: `tests/test_refactor_extracted_routes.py` (7/7 ✓).
- **المتبقّي من التقسيم (P2، تدريجي وآمن)**: server.py لا يزال ~23k سطر و~400 نقطة. يُنصح الاستمرار وحدة-وحدة (مجموعات معزولة مثل login-backgrounds / tables / categories...) مع `restart backend` + اختبار curl بعد كل استخراج. وكذلك تقسيم `Settings.js` (~8k سطر).

---



## Session: 11 Jun 2026 (تكملة 9) — PWA (تطبيق قابل للتثبيت)

- **PWA كان مُفعّلاً مسبقاً ومُتحقَّق**: manifests منفصلة (admin/driver/customer/menu) بـ display:standalone + أيقونات (192/512 + maskable) + scope، تبديل ديناميكي للـmanifest حسب المسار في index.html، وسوم iOS (apple-mobile-web-app-capable + apple-touch-icon + theme-color)، وservice workers (sw-offline/customer/driver/push) مُسجّلة. تحقّقت أن كل الملفات تُخدَّم HTTP 200 عبر الرابط الحيّ.
- **إضافة جديدة — زر "تثبيت التطبيق" داخل التطبيق** (`components/InstallPWAButton.jsx`): يلتقط حدث `beforeinstallprompt` (Android/Chrome/Desktop) ويعرض زراً يبدأ التثبيت بنقرة؛ وعلى iOS يعرض تعليمات "إضافة إلى الشاشة الرئيسية". يختفي تلقائياً إذا كان التطبيق يعمل بملء الشاشة (مثبّت). أُضيف لهيدر Dashboard (الأدمن) وهيدر DriverApp (السائق).
- ملاحظة: زر التثبيت يظهر فقط عندما يقرّر المتصفح أن التطبيق قابل للتثبيت (لا يمكن إجباره في اختبارات headless) — المكوّن قياسي ويُعيد null بأمان عند عدم التوفّر.

---


## Session: 11 Jun 2026 (تكملة 8) — إنهاء التجاوب (إصلاح صفوف أزرار الهيدر)

- شُخّص العنصران المتبقّيان (iteration_235): صفوف أزرار الهيدر في **Dashboard** و**Orders** كانت `flex` بلا التفاف (أزرار whitespace-nowrap) فتتجاوز عرض الشاشة وتُقصّ.
- الإصلاح: جُعلت صفوف الأزرار والحاوية الأم `flex-wrap` مع `gap-2 sm:gap-3` — تلتفّ على الهاتف وتبقى سطراً واحداً على سطح المكتب.
- **اختبار نهائي (iteration_236): 100%** — `body.scrollWidth == innerWidth == 390` على /dashboard و/orders (صفر overflow أفقي)، وسطح المكتب غير متأثّر.
- ✅ **التصميم المتجاوب لكامل النظام مكتمل ومُتحقَّق**: POS + Reports + Delivery + Settings + Orders + Dashboard + حارس عام لكل الصفحات + حوارات 95vw.

---


## Session: 11 Jun 2026 (تكملة 7) — التصميم المتجاوب Phase 2 (واسع، كل النظام)

- **حارس عام ضد كسر الصفحة أفقياً** (index.css): `overflow-x: clip` على html/body تحت 1024px — يحمي **كل الصفحات (40+)** من التمرير/الكسر الأفقي على الهاتف والتابلت.
- **حوارات متجاوبة عامّة**: قاعدة CSS تجعل كل نوافذ shadcn/Radix بعرض 95vw على الهاتف (≤640px) — تغطّي كل النوافذ المنبثقة دفعة واحدة.
- **Reports / Delivery / Settings / Orders**: هيدر ومسافات متجاوبة (px-4 sm:px-6)، شبكة إحصائيات Orders صارت `grid-cols-2 sm:3 lg:5`، جدول سجل التحصيلات `min-w-[760px]` للتمرير الأفقي.
- **اختبار (iteration_234): 100%** على المعايير — صفر overflow أفقي على /dashboard، /reports، /delivery، /settings، /orders (هاتف 390px + تابلت 768px)، الحوار يتّسع 95vw داخل الشاشة، سطح المكتب سليم.
- ملاحظة بسيطة مؤجّلة: عنصر داخلي في Dashboard (≈1037px) يُقصّ على الهاتف دون كسر الصفحة (غالباً صف قابل للتمرير).

---


## Session: 11 Jun 2026 (تكملة 6) — التصميم المتجاوب (Phase 1) + عدّاد طابور الطلبات

- **عدّاد طابور الطلبات الواردة**: شارة في شاشة المكالمة الواردة [incoming-queue-counter] تعرض عدد الطلبات الإضافية المنتظرة ("X طلبات إضافية بانتظارك").
- **مخطّط تجاوب شامل** أُنشئ عبر design_agent في `/app/design_guidelines.json` (نقاط الكسر، نمط الهاتف، الجداول، الحوارات، القوائم) مع أولوية تنفيذ من 6 مراحل.
- **Phase 1 — شاشة POS متجاوبة بالكامل** (مُختبَرة 100% على 390px/768px/1920px، بلا overflow أفقي): التخطيط ثلاثي الأعمدة (فئات/منتجات/سلة) يتراصّ عمودياً تحت `lg` (الفئات شريط علوي أفقي قابل للتمرير، المنتجات شبكة عمودين، السلة بعرض كامل أسفل)، ويبقى ثلاثي الأعمدة على الشاشات الكبيرة (≥1024px). الهيدر يلتفّ على الشاشات الصغيرة. (iteration_233)
- **Reports**: جدول سجل التحصيلات صار `min-w-[760px]` داخل overflow-x-auto (يمرّر أفقياً بدل التزاحم على الهاتف)، والهيدر صار متجاوباً (padding/نص متجاوب). باقي الجداول ملفوفة مسبقاً.
- **متبقٍ (مراحل قادمة حسب design_guidelines.json)**: حوارات `w-[95vw]` على الهاتف (Phase 4)، تجاوب إدارة التوصيل (ارتفاع الخريطة/اللوحات) (Phase 5)، نماذج الإعدادات المتراصّة (Phase 6)، وبقية الصفحات (~35) تدريجياً.

---


## Session: 11 Jun 2026 (تكملة 5) — طابور الطلبات الواردة (FIFO) + زر إعادة طباعة الفاتورة

- **طابور الطلبات الواردة FIFO** (طلب المستخدم: لا مهلة تلقائية، بل بالترتيب): `IncomingOrderCall.jsx` كان يعرض الأحدث أولاً (الإشعارات تأتي مرتّبة DESC من الخادم). الآن تُرتَّب تصاعدياً حسب `created_at` ويُختار **الأقدم غير المعالَج** أولاً — يظهر طلب واحد في كل مرة، وبعد قبول/رفض الحالي يظهر التالي بالترتيب تلقائياً (بلا مهلة).
- **زر "إعادة طباعة الفاتورة"** داخل نافذة تفاصيل الطلب (إدارة التوصيل): يطبع فاتورة أي طلب (حتى المرفوض/المسلّم) عبر طابعة USB إن وُجدت (`printSavedOrder`)، وإلا عبر نافذة طباعة المتصفح. (اختبار iteration_232: 100% — يعمل للطلب العادي والمرفوض #5002.)

---


## Session: 11 Jun 2026 (تكملة 4) — تفاصيل الطلب القابلة للضغط + إصلاح كلفة المواد في سجل التحصيلات

- **كل الطلبات في إدارة التوصيل قابلة للضغط** (طلب المستخدم P1): الضغط على أي طلب (تبويب "كل الطلبات"، وأعمدة في الطريق/المسلّمة/حركة اليوم) يفتح نافذة تفاصيل كاملة [order-details-dialog]: رقم الطلب، الحالة مع وسم "مرفوض"، السعر/الإجمالي، **المنتجات**، **اسم الكاشير**، وقت الإنشاء، و**للمرفوضة**: سبب الرفض + وقت الرفض. يجلب التفاصيل عبر `GET /api/orders/{id}`. أزرار الإجراءات داخل بطاقة "في حيازة السائق" تستخدم stopPropagation. (اختبار iteration_231: 100% 3/3.)
- **إصلاح backend لعرض التفاصيل**: `GET /api/orders/{id}` كان `response_model=OrderResponse` يحذف حقول الرفض → أُضيفت للنموذج (is_rejected/cancellation_reason/cancelled_at/rejected_at/rejected_by_name/cashier_name)، ويُحلّ cashier_name من cashier_id إن غاب.
- **إصلاح "إجمالي كلفة المواد = 0" في سجل التحصيلات** (صورة المستخدم): السجلات القديمة لم تخزّن total_materials_cost فظهر السطر صفراً. الآن `GET /api/reports/delivery/collections` يحسب كلفة المواد لكل تحصيل **ديناميكياً** من `order_ids` → طلباته → أصنافه (نفس مصدر تقرير المبيعات). مُتحقَّق: تحصيل بـ5 طلبات أظهر كلفة مواد محسوبة بدل الصفر.

---


## Session: 11 Jun 2026 (تكملة 3) — إصلاحات P1

- **إصلاح `PUT /api/drivers/{id}`**: النقطة الفعّالة (`routes/drivers_routes.py`) كانت لا تحفظ `is_available`/`current_order_id`. أُضيفا لنموذج `DriverUpdate` ولمنطق التحديث (سلسلة فارغة لـcurrent_order_id = تفريغ إلى null). مُتحقَّق عبر curl: is_available=True يُحفظ، current_order_id يُضبط ويُفرَّغ بنجاح.
- **عكس تدفّق أعمدة تبويب "طلبات جاهزة للتوصيل" لـRTL**: أُضيف `dir="rtl"` لحاوية الشبكة، فصار العمود الأول (جاهزة للتعيين) يمين الصفحة و"حركة اليوم" يسارها — يتماشى مع القراءة العربية.

---


## Session: 11 Jun 2026 (تكملة 2) — توحيد الخرائط + تبويبات إدارة التوصيل

- **الخريطة مثل تطبيق Baly** (صورة المستخدم): بُدِّلت كل بلاطات الخرائط من CARTO Voyager الباهتة إلى **OpenStreetMap القياسية التفصيلية** (`https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png`) — تُظهر الشوارع وأسماءها ومعالم (مستشفيات/مدارس/حدائق) بدقة واحترافية. شملت: `DriverTrackingMap.js` (خريطة إدارة التوصيل، النمط streets)، `CustomerMenu.js` (موضعان)، `PublicTracking.jsx`، `DriverApp.js`. (مُتحقَّق: بلاطة OSM تُرجع HTTP 200.)
- **تبويب "كل الطلبات" يستبعد طلبات شركات التوصيل** (صورة 10:16م): `/api/delivery-orders` صار يتخطّى أي طلب شركة توصيل (is_delivery_company/delivery_company_name/delivery_app_name/delivery_app_id)، فيعرض فقط طلبات تطبيق الزبائن + التوصيل الداخلي. (التحقق: summary.all هبط من 20 إلى 14.)
- **تبويب "طلبات جاهزة للتوصيل" — 4 أعمدة دقيقة** (صورة 10:16م): أُعيد بناء الأعمدة لتعتمد على البيانات الشاملة (allOrders) بدل بيانات السائق المُختار: (1) جاهزة للتعيين، (2) في حيازة السائق (out_for_delivery)، (3) **الطلبات المسلّمة للزبون (جديد)** قبل (4) حركة اليوم (تسلسل كل طلبات اليوم). بطاقات الإحصائيات العليا (في الطريق/تم التسليم اليوم) صارت تعتمد allOrders/ordersSummary. أُضيف `driver_id/driver_phone/driver_payment_status/payment_status` لمخرجات `/delivery-orders`.
- الاختبار (iteration_230): Frontend + Backend 100% (الاستبعاد + الأعمدة الأربعة + البطاقة الجديدة). ملاحظة تصميمية بسيطة: ترتيب الأعمدة الأربع يتدفّق LTR داخل صفحة RTL (الترتيب في DOM صحيح: المسلّمة قبل الحركة) — بانتظار رأي المستخدم إن أراد عكس التدفّق بصرياً.

---


## Session: 11 Jun 2026 (تكملة) — تعليقات الصور #397 (4 تغييرات) + إصلاح بيئة

- **أجور التوصيل كبند في السلة والفاتورة** (صورة 8:43م): في الـPOS تظهر أجور التوصيل الآن كسطر مستقل (مثل الخصم) أسفل سطر الخصم وقبل الإجمالي — في ملخّص السلة (`cart-delivery-fee-line`)، وفي معاينة الإيصال (`receipt-delivery-fee-line`)، والإجمالي يشمل الأجور (`grandTotalWithDelivery`). وفي الفاتورة المطبوعة (USB bitmap `receiptBitmap.js`) أُضيف سطر "أجور التوصيل: +المبلغ" بعد الخصم. `buildPrintOrderData` يمرّر `delivery_fee` + `subtotal`.
- **الطلب يبقى معلّقاً حتى التحصيل** (صورة 8:45م): تعديل `fetchPendingOrders` في `POS.js` — طلبات التوصيل الداخلي المُسلَّمة (status=delivered) تبقى ضمن "الطلبات المعلّقة" طالما `driver_payment_status != 'paid'` و`payment_status` ليست مدفوعة، فلا تختفي بمجرد ضغط السائق "تم الاستلام/التسليم"، بل بعد تحصيل الكاشير أو الدفع من نقطة البيع.
- **طباعة إيصال عند التحصيل من السائق** (توضيح المستخدم: "اريده يطبع لأنه لا يطبع"): زر "تحصيل المبلغ" في سجل طلبات السائق (`Delivery.js`) صار يفتح نافذة طباعة بإيصال "محصّلة من السائق فلان" (قائمة الطلبات + الإجمالي + المبلغ المُحصّل) بعد نجاح `collect-payment`.
- **سطر إجمالي سجل التحصيلات** (صورة جديدة 8:06م): تذييل جدول "سجل التحصيلات" في تقرير التوصيل صار يعرض 5 إجماليات (يسار→يمين): إجمالي المبيعات، إجمالي كلفة المواد، إجمالي الاستقطاعات (العمولة)، العرض/الفرق، الإجمالي المُحصّل. أُضيف حقل `total_materials_cost` لنموذج `DeliveryCollectionCreate` ولسجل `delivery_collections` (backend)، ويُمرَّر من الواجهة عند التحصيل الفردي والجماعي.
- إصلاح بيئة الـfork: إعادة تثبيت تبعيات backend (uvicorn) + تشغيل MongoDB يدوياً. أيضاً أُزيلت بقايا JSX تالفة في نهاية `Reports.js` (من جلسة سابقة مقطوعة) كانت تمنع التحويل.
- الاختبار (iteration_228): Backend 100% (collect يقبل total_materials_cost، collections يعيد الحقول، /orders سليم) + سطر إجمالي سجل التحصيلات يعرض الـ5 testids. POS delivery-fee + تحصيل السائق: مراجعة كود + النقاط الخلفية سليمة.

---


## Session: 11 Jun 2026 (تكملة) — تصحيحات حسب تعليقات الصور

- **تصحيح حرج:** الطلب المرفوض كان يظهر في "إدارة الطلبات" كملغي — صار يُستبعَد. `GET /api/orders` يستبعد `is_rejected` افتراضياً، ويعرضها فقط عبر `include_rejected=true` (لإدارة التوصيل). (صور #4/#6/#7)
- **توحيد الخريطة:** خريطة "إدارة التوصيل" (`DriverTrackingMap.js`) صارت تستخدم بلاطات CARTO Voyager نفسها المستخدمة في تطبيق الزبائن والسائقين. (صور #1/#8)
- **أزرار بطاقة الطلب** (`Orders.js`): نُقلت لأسفل البطاقة في صفّ يلتف ومتمركز (`flex-wrap justify-center`) بدل أن تُقصّ. (صورة #9)
- **عرض المنتجات** في شاشة قبول الطلب (`IncomingOrderCall.jsx`): قائمة بأسماء المنتجات + الكمية + السعر لكل صنف. (صور #10/#16/#21)

### متبقٍّ من تعليقات الصور
- إدارة التوصيل: عرض شامل لكل الطلبات (مرفوض + متأخر القبول >دقيقتين + مقبول + مكتمل + قيد التحضير).
- إشعار التقييم للزبون عند التسليم / عند فتح التطبيق لاحقاً (صورة #2).

---


## Session: 11 Jun 2026 — إعادة تصميم إشعار الكاشير + إرسال للتحضير + رفض/تقييم/سجل التوصيل

- **إعادة تصميم إشعار "طلب جديد وارد" للكاشير** (`IncomingOrderCall.jsx`): إزالة شكل المكالمة (السماعة/الأزرار الدائرية) واستبدالها بشعار التطبيق `BrandLogo` + زرّين مربّعين بزوايا دائرية "قبول الطلب" (أخضر) / "رفض الطلب" (أحمر). (طلب المستخدم في صورة #1931)
- **شاشة ما بعد القبول** (صورة #1932): أجور توصيل تلقائية حسب المسافة + زرّا (+/−) لتعديلها + **اختيار سائق إجباري** + زر **"إرسال للتحضير"** بدل "لاحقاً".
- **زر "إرسال للتحضير"**: يُسند السائق + الأجور، يطبع **فاتورة الكاشير USB + تذاكر المطبخ** (إعادة استخدام `printService.printSavedOrder`، مع طباعة احتياطية بالمتصفح)، يضبط `kitchen_status=pending_kitchen` (ظهور فوري بالمطبخ)، ويبقى الطلب في المعلّق.
- **إشعارات السائق**: عند الإسناد يُنشأ إشعار `new_order_driver` (في `routes/drivers_routes.py`)، وعند ضغط المطبخ "جاهز" يُنشأ `order_ready` (في `server.py` kitchen-status). `DriverApp.js` يستطلع `/order-notifications?driver_id=` كل 5 ثوانٍ ويعرض توست مميز لكل نوع.
- **رفض الطلب**: نقطة جديدة `PUT /api/orders/{id}/reject` تحوّل الطلب إلى `cancelled` مع `is_rejected/cancellation_reason/rejected_by_name` (لا يُحذف) فيظهر في بطاقة "ملغي" بصفحة الطلبات، ويُشعَر الزبون. `reject()` في الإشعار يستدعيها بعد تأكيد.
- **ترجمة**: إضافة `out_for_delivery` → "في الطريق" في `Orders.js` (اللون/الأيقونة/النص).
- **تقييم الزبون بعد التسليم**: `POST /api/track/{id}/rating` (عام، الطعام/المطعم/السائق 1-5 + ملاحظات، يمنع التكرار) + علَم `is_rated` في `order-driver-info`. نموذج نجوم في `PublicTracking.jsx` يظهر عند `status=delivered`.
- **سجل التوصيل للمالك**: `GET /api/delivery-ratings` (متوسطات + قائمة) + تبويب جديد "سجل التقييمات" في `Delivery.js`.
- اختبارات: `backend/tests/test_delivery_rating_reject.py` (5/5 ✅). تحقق بصري بلقطات لكل الشاشات. ملاحظة: طباعة USB تتطلب وسيط الطباعة الفعلي (يسقط للطباعة بالمتصفح في غيابه).

---


## Session: 08 Jun 2026 (تكملة) — رقم طلب شركة التوصيل أصبح إجبارياً (ضمان 100٪ ذاتي)

- بناءً على طلب المستخدم أن يمنع النظام التكرار 100٪ بنفسه (الزر للطوارئ فقط): جُعل حقل **"رقم الطلب لدى شركة التوصيل" إجبارياً عند اختيار شركة توصيل** (طلبات/مزاجك) في الـPOS (`POS.js`) — في مساري "حفظ معلّق" و"الدفع" (التحقق عند `deliveryApp && !deliveryCompanyOrderId.trim()`). توصيل السائق الخاص (بلا شركة) لا يتأثر.
- الحقل الآن بعلامة `*` حمراء وحدود حمراء/خضراء حسب التعبئة (`data-testid=delivery-company-order-id`).
- بهذا: طلب أي شركة توصيل يحمل رقماً خارجياً دائماً → منع التكرار الخادمي (409 أونلاين / idempotent أوفلاين) يضمن **استحالة التكرار رياضياً من أي جهاز/وضع**.
- التغطية الكاملة الآن: نفس الجهاز 100٪ (مفتاح ثبات + دورة حياة)؛ طلبات الشركات 100٪ (رقم إجباري + منع خادمي)؛ الزر المتقدم للطوارئ/التراكمات القديمة فقط.
- صحة: POS يُحمّل بلا أخطاء (smoke). أخطاء lint الأربعة (POS.js:189/724/1025/1546) سابقة لتعديلاتي (react-compiler على كود قائم).

---


## Session: 08 Jun 2026 (تكملة) — أداة كشف وتنظيف التكرارات المتقدمة (واجهة)

- ترقية حوار "تنظيف الطلبات المكررة" في صفحة إدارة الطلبات (`Orders.js`): أُضيف **قسم فحص متقدم** يستدعي `GET /sync/business-duplicate-orders` ويعرض **معاينة** لمجموعات التكرار (نفس رقم طلب الشركة / محتوى متطابق) مع تحديد أي طلب يُبقى، وزر **"تنظيف تلقائي"** يستدعي `POST /sync/cleanup-business-duplicates`.
- يحلّ شكوى المستخدم أن الأداة القديمة كانت تقول "لا توجد تكرارات" بينما هي موجودة: الآن تكتشف تكرار رقم طلب التوصيل (مثل 9242) الذي كان يفوته الكشف القديم.
- الاختبار: بذر طلبين بنفس الرقم الخارجي (9242) → الحوار عرض المجموعة (#8001/#8002 → يُبقى #8001) + مجموعة محتوى متطابق؛ التنظيف حذف 2 وأبقى نسخة لكل مجموعة ثم 0 تكرار. lint: الخطأ الوحيد (Orders.js:314 updateOrderStatus) سابق لتعديلاتي.

---


## Session: 05 Jun 2026 (إصلاح جذري P0) — القضاء على تكرار الطلبات أونلاين/أوفلاين نهائياً

### السبب الجذري (لماذا تكرر رغم الإصلاحات السابقة)
- الإصلاحات السابقة منعت نوعاً واحداً فقط: نفس `offline_id` يُرسَل مرتين. لم تمنع: (أ) نفس طلب التوصيل من جهازين/إعادة إدخال (مفاتيح مختلفة)، (ب) طلب معلّق أوفلاين ثم يُدفع بمفتاح جديد بينما النسخة القديمة تُزامَن، (ج) لا يوجد التقاط لرقم طلب شركة التوصيل (9242) كحقل مستقل فلا الفهرس ولا أداة التنظيف يرونه.
- الترقيم الفوضوي: رقم الطلب يُمنَح وقت المزامنة لا وقت الإنشاء.

### الحل (نظام منع تكرار متعدد الطبقات)
1. **مفتاح ثبات يرافق الطلب طوال حياته** (`POS.js`): `clientOrderId = editingOrder?.offline_id || generate()` — لا يتولّد مفتاح جديد عند الدفع/التعديل → يقتل تكرار "المعلّق ثم الدفع".
2. **حقل "رقم الطلب لدى شركة التوصيل"** (اختياري) في الـPOS (`data-testid=delivery-company-order-id`)، يُرسَل في كل مسارات الطلب. أُضيفت حقول `delivery_company*` لنموذج `OrderCreate` (كانت مفقودة فيُسقطها Pydantic).
3. **منع فريد على رقم الطلب الخارجي** (`server.py` create_order + `sync_routes.py` sync_order): نفس (الشركة + رقم الطلب) → أونلاين يرجع **409 DUPLICATE_DELIVERY_ORDER** برسالة واضحة، وأوفلاين يُعيد الموجود (idempotent). الـPOS يعرض تحذيراً ولا يُنشئ نسخة.
4. **إصلاح خطأ كامن (HIGH)**: بصمة المحتوى (30ث) كانت تتجاهل رقم الطلب الخارجي فقد تَدمج طلبَي توصيل مختلفين بنفس الأصناف → **فقدان طلب**. الآن البصمة تتضمّن `payment_method` + `delivery_company_order_id` + حماية إضافية تمنع الدمج عند اختلاف الرقم الخارجي.
5. **ترقية أداة الكشف**: `GET /sync/business-duplicate-orders` + `POST /sync/cleanup-business-duplicates` تكتشف التكرار التجاري (نفس الرقم الخارجي + بصمة محتوى) لتنظيف التراكمات القديمة.

### الاختبار (iteration_219)
- Backend: 8/8 اختبارات منع تكرار + 14/14 regression (الكابتن + تقرير التوصيل) — كلها PASS.
- تحقّق نهائي بعد إصلاح الخطأ الكامن: ext مختلف (EXTA #25 / EXTB #26) ينشئ طلبين متمايزين (لا فقدان)، ونفس ext يُمنع (409). Frontend: الحقل موصول في 4 مسارات + معالجة 409 في الـPOS (مراجعة كود؛ المعاينة حُجبت بشاشة البداية).

### القرار للمستخدم: حقل رقم التوصيل **اختياري** + عند التكرار **يُمنع** (أ2 + ب1).
...
## 2026-06-12 (fork iter59) — إشعارات Web Push حقيقية + مكالمة الطلب للكاشير فقط + تنبيهات الإدارة عند التأخير/الرفض
### P0: Web Push حقيقي (VAPID + pywebpush) لتطبيق السائق (مثل Toters)
- وُلِّدت مفاتيح VAPID وحُفِظت في backend/.env. نقطة نهاية GET /api/push/vapid-public-key.
- `send_push_notification` صار يُرسل إشعاراً حقيقياً عبر pywebpush.webpush() مع توقيع VAPID (كان وهمياً يسجّل بالـDB فقط)، ويُعطّل الاشتراكات المنتهية (404/410).
- إشعار فوري للسائق عند إسناد طلب (assign-driver) وعند رسالة محادثة جديدة من الزبون.
- `sw-driver.js`: أُضيف معالج push + notificationclick (يعمل بالخلفية حتى عند إغلاق التطبيق). تطبيق السائق يشترك تلقائياً عند تسجيل الدخول/استعادة الجلسة.
- تحقق: pytest 3/3 + خدمة الدفع أرجعت 410 (أي وُثِّق توقيعنا بنجاح).
### مكالمة الطلب الوارد للكاشير فقط + تصعيد للإدارة (طلب المستخدم — صورة CF3D)
- IncomingOrderCall: ALLOWED_ROLES صارت ['cashier'] فقط (كانت تشمل admin/manager/owner). الإدارة لم تعد ترى المكالمة.
- نقطة نهاية GET /api/order-notifications/escalations: إذا لم يوافق الكاشير على طلب خلال 5 دقائق → يُنشأ تنبيه 'order_management_alert' (not_approved) باسم الفرع + اسم الكاشير في الوردية + تفاصيل الطلب. عند رفض الكاشير → تنبيه (rejected).
- مكوّن ManagementOrderAlerts (أسفل-يسار) يعرض هذه التنبيهات للإدارة (admin/manager/owner/super_admin/supervisor).
- تحقق: التصعيد بعد 5 دقائق يعمل end-to-end (curl) باسم الفرع/الكاشير/التفاصيل.
### تحديث شبه فوري في إدارة التوصيل
- تقليل فترات الاستطلاع: fetchData 30s→10s، fetchAllOrders 20s→8s ليختفي الطلب المحذوف/المدفوع/المحوّل بسرعة دون إعادة تحميل.

## 2026-06-12 (fork iter59-b) — إصلاح تبديل لغة قائمة الزبائن (فوري بلا إعادة تحميل) + ترجمة أسماء المنتجات/الأقسام
- `useTranslation.changeLanguage`: حُذف `window.location.reload()` — التبديل صار فورياً (SPA). المكوّنات الأخرى تلتقط اللغة خلال ~0.5ث عبر استطلاع localStorage. (شكوى المستخدم: "عند اختيار الإنجليزية يعيد التحميل ولا يحوّل").
- أُضيف `localName(obj)` للهوك: يُرجع name_en عند اختيار الإنجليزية مع رجوع للاسم العربي. طُبِّق على أسماء المنتجات والأقسام في CustomerMenu (categories + product cards).
- تحقق: عند الضغط EN لا يحدث reload (URL ثابت)، dir=ltr lang=en، ونصوص إنجليزية ثابتة تظهر ✓.
- ملاحظة: ترجمة أسماء المنتجات تتطلب إدخال name_en لكل منتج من إعدادات المالك (حالياً 1/8 فقط لديه name_en) — البنية جاهزة، يتبقى إدخال البيانات/حقل الإدخال في الإعدادات.

## 2026-06-12 (fork iter60) — تدقيق تبويب "كل الطلبات" + إصلاح زر التواصل مع السائق
### تبويب "كل الطلبات" بإدارة التوصيل (GET /api/delivery-orders)
- تأكيد: يعرض طلبات تطبيق الزبائن + التوصيل الداخلي (سائق المطعم) ويستبعد طلبات شركات التوصيل (توترز/طلبات...). (مُثبَت باختبار حي).
- **إصلاح خطأ في الشارات:** كان `summary.accepted` يَعُدّ الطلبات المعلّقة (بانتظار القبول) ضمن "مقبول" بينما قائمة التبويب تُفلتر حسب category — فلا تتطابق الأعداد. الآن العدّ يستخدم نفس تصنيف category تماماً، فتتطابق شارات (مرفوض/مقبول/قيد التحضير/مكتمل/متأخر القبول) مع القوائم. الطلب المعلّق يظهر في "الكل" فقط حتى يصبح متأخراً.
- اختبار regression: tests/test_delivery_all_orders_iter60.py (يشمل/يستبعد + تطابق الأعداد).
### زر التواصل مع السائق بتتبّع الزبون (CustomerMenu — صورة IMG_2650)
- حُذف الخيار الوهمي "اتصال داخل التطبيق (قادمة قريباً)" الذي كان "لا يعمل ولا يرسل شيئاً".
- خيارات تعمل فعلياً ومُرتّبة: اتصال واتساب 🟢، رسالة واتساب 🟩، اتصال هاتفي 📞، SMS، محادثة داخل التطبيق 💬.
- `tel:`/`sms:` تستخدم الأرقام المنظّفة (بلا فراغات)؛ روابط واتساب فقط تفتح بتبويب جديد. رسالة واضحة عند غياب رقم السائق (data-testid=driver-no-phone-hint).
- ملاحظة: روابط tel:/واتساب تعمل على الهاتف فقط (لا تفعل شيئاً على المتصفح المكتبي).

## 2026-06-12 (fork iter61) — ترجمة AI + إشعارات الزبون + تقييم + باركود QR
### ترجمة تلقائية لأسماء المنتجات/الأقسام (name_en) بالذكاء الاصطناعي
- نقطة نهاية POST /api/admin/translate-names: تترجم أسماء المنتجات والأقسام العربية إلى الإنجليزية وتخزّنها كـ name_en (OpenAI gpt-4o عبر emergentintegrations / EMERGENT_LLM_KEY). overwrite اختياري.
- زر "ترجمة تلقائية (EN)" في إعدادات المنتجات (data-testid=auto-translate-btn) — يترجم الكل دفعة واحدة ثم يحدّث.
- تحقق: تُرجم 8 منتجات + 5 أقسام؛ قائمة الزبون بالإنجليزية تعرض Burger/Pizza/Drinks/Desserts/Salads ✓.
### إشعارات Push للزبون عند جاهزية الطلب + تقييم بعد التسليم
- إصلاح: اشتراك الزبون كان يستخدم مفتاح VAPID تجريبياً (Google sample) → يجلب الآن المفتاح الحقيقي من /push/vapid-public-key.
- ربط إشعار الزبون بنقطة تحديث الحالة من الكاشير (PUT /orders/{id}/status) — كان لا يُشعر الزبون. الآن يُرسل: جاري التحضير/جاهز/في الطريق/تم التسليم.
- إشعار "تم التسليم" يحمل rate:true؛ نافذة التقييم تُفتح تلقائياً في تطبيق الزبون عند التسليم (مرة واحدة، يُعلَّم is_rated). نقطة /customer/rate-order موجودة مسبقاً.
- تحقق: إشعارات ready/delivered تُسجّل بنجاح؛ rate-order يعمل ويضبط is_rated ✓.
### باركود QR على إيصال التوصيل
- إيصال إعادة الطباعة (Delivery) يحتوي الآن على QR يشفّر رابط التتبّع /track/{order_id} للمسح السريع (مكتبة qrcode).
### مؤجَّل
- (P2) تقسيم server.py (~23k سطر) و Settings.js — مهمة كبيرة وعالية الخطورة، تحتاج جلسة مخصّصة مع اختبار بعد كل خطوة.

## 2026-06-12 (fork iter62) — زر "شارك تجربتك" بعد التقييم (تسويق مجاني)
- بعد إرسال التقييم بنجاح تتحوّل نافذة التقييم إلى شاشة "شكراً + شارك تجربتك" (data-testid=share-experience-view).
- أزرار المشاركة: مشاركة أصلية عبر جهاز الزبون (navigator.share → واتساب/إنستغرام/...) (share-native-btn)، واتساب (share-whatsapp-btn)، إنستغرام (نسخ الرسالة + فتح إنستغرام) (share-instagram-btn)، ونسخ رابط المطعم (share-copy-btn).
- الرسالة جاهزة: "جرّبتُ {اسم المطعم} وكانت تجربة رائعة! {نجوم}\nاطلب الآن: {رابط القائمة /menu/{tenant}}". يحوّل كل تقييم إلى رابط تسويقي يجلب زبائن جدد.
- تحقق: يبني بنجاح + القائمة تُحمّل بلا أخطاء JS.

## 2026-06-13 (fork iter63) — منع تكرار الطلبات نهائياً + صرف الراتب من خزينة المالك
### منع تكرار الطلبات (فرع السيدية #34/#35 وغيره) — دفاع متعدد الطبقات
- **الخادم (الأقوى):** توسيع نافذة منع التكرار ببصمة المحتوى من 30ث → **150ث** (الطلبان كانا بفارق دقيقة فأفلتا). وأُضيف `table_id` للبصمة فلا تُدمَج طاولتان مختلفتان بنفس الأصناف (حماية من فقدان طلب حقيقي).
- **الواجهة (POS):** قفل متزامن (useRef) يمنع الإرسال المزدوج عند الضغط المتكرر (حالة React غير فورية فتتجاوزها الضغطة المزدوجة).
- **أداة "تنظيف المكرر":** إضافة projection لفحص المكرر لتفادي بطء/فشل التحميل ("فشل تحميل الطلبات المكررة") على الفروع المزدحمة.
- تحقق: طلبان متطابقان سريعان → طلب واحد (لا تكرار)؛ اختلاف الطاولة → طلب جديد (لا دمج خاطئ). اختبار انحدار test_order_dedup_iter63.
### صرف الراتب يُخصم من خزينة المالك حسب فرع الموظف (طلب المستخدم)
- كان زر "صرف" (PUT /payroll/{id}/pay) يُعلّم الكشف "مدفوع" فقط بلا أي خصم من الخزينة.
- الآن: يخصم (صافي الراتب − ما صُرف نقداً مسبقاً لنفس الشهر) من خزينة المالك (owner_withdrawals، فئة salary_payment) **حسب الفرع المقيَّد في بطاقة الموظف**، مع التحقق من كفاية رصيد الفرع، ومنع الصرف المزدوج. يُحفظ linked_owner_withdrawal_id + paid_amount.
- تحقق: صرف 500k خُصم من خزينة الفرع ✓؛ صرف مزدوج ممنوع ✓؛ رصيد غير كافٍ محظور ✓. اختبار test_payroll_treasury_iter63.

## 2026-06-13 (fork iter64) — إنهاء خدمات الموظف (End of Service) + الصرف من خزينة المالك
### دورة حياة كاملة لإنهاء الخدمات (طلب المستخدم عبر 4 صور)
- زر "إنهاء خدمات" أحمر لكل موظف نشط في HR > الموظفين (للمدير/المالك فقط). عند الضغط: حالة terminated_pending، **خط أحمر** على الاسم + شارة "إنهاء خدمات"، وجدولة حذف تلقائي بعد 24 ساعة.
- زر أحمر "صرف المستحقات وإنهاء الخدمات": يحتسب صافي الراتب حسب أيام العمل الفعلية حتى تاريخ الإنهاء (− الخصومات − كامل رصيد السلف − ما صُرف نقداً) ويخصمه من **خزينة المالك حسب فرع الموظف** (owner_withdrawals، فئة end_of_service)، مع التحقق من كفاية الرصيد.
- زر "إرجاع للعمل" (خلال 24 ساعة فقط): يعكس الصرف (يحذف السحب فيعود الرصيد للخزينة) ويعيد الموظف نشطاً.
- بعد 24 ساعة (process_terminations يُستدعى عند GET /employees): terminated_pending → terminated (يُخفى من القائمة، is_active=False فيُزال من جهاز البصمة عبر المزامنة، pending_device_removal=True، archive_purge_at=نهاية الشهر التالي). لا يقبل الإرجاع.
...
## 2026-06-30 — تصحيح كمية/سعر صنف في فاتورة مُستلمة + حارس كمية ضخمة (P0) ✅
- **المشكلة:** فاتورة #4 على الإنتاج فيها "اورغانو 7999 كغم = 71,991,000" بدل 7.999 كغم (ضاعت الفاصلة العشرية ×1000)، والفاتورة مُستلمة+مدفوعة فلا سبيل لتصحيحها (لا تعديل، والحذف ممنوع للمُرسَل للمخزن). أدى لتضخيم: إجمالي الفاتورة + مخزون المادة + رصيد المورد + سحب الخزينة.
- **Backend (inventory_system.py):** نقطة جديدة `PATCH /api/purchases-new/{id}/correct-item` (admin/super_admin/manager). تُعدّل كمية/سعر صنف وتعكس كل الآثار: إعادة حساب إجمالي الفاتورة + تعديل raw_materials (الكمية + متوسط التكلفة بطريقة القيمة) + تعديل supplier.total_purchases بالفرق + استرجاع المبلغ المدفوع الزائد للخزينة (تقليل/حذف owner_withdrawals) + تحديث warehouse_transactions + تسجيل audit_log.
- **Frontend (PurchasesPage.js):** زر تصحيح (قلم) بجانب كل صنف في نافذة التفاصيل (للمالك/المدير) → نافذة "تصحيح صنف" (كمية صحيحة + سعر اختياري + سبب) مع حساب حيّ للإجمالي الجديد وتنبيه بالعكس التلقائي. + حارس تأكيد عند كمية > 1000 لوحدات الوزن/الحجم (كغم/غرام/لتر...) لمنع خطأ الفاصلة العشرية مستقبلاً (إضافة للحارس الموجود >10M على الإجمالي).
- **التحقق:** اختبار e2e كامل (curl/python): إنشاء فاتورة 7999 كغم مُستلمة+مدفوعة 72,015,000 → تصحيح إلى 7.999 → إجمالي 95,991، مخزون اورغانو 7.999، رصيد المورد 95,991، استرجاع خزينة 71,919,009، الحالة paid. + لقطات شاشة تؤكد الزر والنافذة والحساب الحيّ (71,991).
- **⚠️ يتطلب Save to GitHub + نشر** لاستخدامه على maestroegp.com لتصحيح فاتورة #4 فعلياً (لا يمكن تعديل قاعدة بيانات الإنتاج من بيئة التطوير).

## 2026-06-30 — إصلاح صورة الفاتورة المكسورة في نافذة التفاصيل ✅
- **المشكلة:** صورة الفاتورة في نافذة تفاصيل المشتريات تظهر مكسورة للصور القديمة المخزّنة بمسار /uploads/ (بلا /api) — الـ ingress يوجّه /uploads/ إلى الواجهة (يرجع index.html بدل الصورة).
- **الإصلاح (PurchasesPage.js):** أُضيفت دالة imgUrl() (تطبيع /uploads/ → /api/uploads/) ومكوّن InvoiceImage مع بديل لطيف عند تعذّر التحميل (بدل أيقونة الصورة المكسورة). استُبدل بناء الرابط القديم API.replace('/api','').
- **التحقق:** testing_agent iter260 (frontend 4/4 = 100%): صورة IMG-OLD (مسار قديم) وIMG-NEW (مسار جديد) تُعرضان 200 image/png؛ + curl يؤكد /uploads/ يرجع text/html بينما /api/uploads/ يرجع image/png. وميزة تصحيح الصنف تعمل من الواجهة (72,015,000 → 95,991، استرجاع خزينة 71,919,009).
- **⚠️ يتطلب Save to GitHub + نشر** للظهور على maestroegp.com.

## 2026-06-30 — منع ازدواج الورديات + مطابقة الإغلاق للمبيعات (P0 حرج) ✅
- **المشكلة (فرع اليرموك فقط):** ظهر شفتان مغلقان لنفس الاسم «محمد صبحي» كلٌّ ~1.9M بينما مبيعات اليوم الحقيقية ~1.9M → ازدواج حساب المبيعات. السبب الجذري: حارس فتح الشفت كان يفحص cashier_id فقط، فسمح بوجود شفتين مفتوحين بنفس الاسم (معرّفان مختلفان/مسارات فتح مختلفة)، وحساب الإغلاق الزمني ضاعف نفس الطلبات.
- **الإصلاح (shifts_routes.py):** helper _open_shift_conflict + _conflict_msg يمنع فتح أي وردية جديدة إذا وُجدت وردية مفتوحة **بنفس اسم الكاشير** (منظّم) داخل نفس الـ tenant — طُبّق على كل مسارات الفتح: POST /shifts (400)، /shifts/open، /shifts/open-for-cashier، /shifts/auto-open (blocked). لا يُحظر حسب الفرع وحده حتى لا نعطّل الفروع متعددة الكاشير (أسماء مختلفة). أُضيف tenant_id لوثيقة الشفت في open_shift (كان مفقوداً ويكسر الحارس).
- **مطابقة الإغلاق للمبيعات:** close_shift يحسب الطلبات بـ shift_id (كل طلب في وردية واحدة) + توافق خلفي للطلبات بلا shift_id. (close_cash_register و day-management/close كانا يعتمدان shift_id مسبقاً.) آمن تماماً للفروع الطبيعية (نفس النتيجة لشفت واحد).
- **Frontend (Dashboard.js):** openShiftForCashier يعرض toast.error عند blocked بدل الفشل الصامت.
- **التحقق:** testing_agent iter261 + pytest /app/backend/tests/test_shift_guard_and_close.py = **4/4 PASS**: (منع المكرر بنفس الاسم / إرجاع شفت نفس الكاشير / السماح باسم مختلف / إجمالي الإغلاق = طلبات الشفت فقط).
- **⚠️ يتطلب Save to GitHub + نشر** لتفعيله على maestroegp.com. الحل يمنع التكرار مستقبلاً في أي فرع/عميل؛ السجلات المزدوجة القديمة على الإنتاج تبقى تاريخية (لا يمكن تعديل قاعدة بيانات الإنتاج من هنا).

## 2026-07-02 — السبب الجذري الحقيقي لازدواج الورديات: مساران للإنشاء التلقائي ✅
- **اكتشاف:** بعد النشر #449 استمرت المشكلة لأن الحرّاس السابقة غطّت مسارات الفتح الصريحة فقط. المصدر الحقيقي: مساران يُنشئان شفتاً تلقائياً دون فحص الاسم:
  1. GET /api/cash-register/summary (فتح نافذة "إغلاق الصندوق" — خاصة عند "وضع المعاينة كـ كاشير" أو وجود حسابين بنفس الاسم): كان يفحص cashier_id فقط ثم يُنشئ شفتاً جديداً.
  2. POST /api/orders (إنشاء طلب بلا شفت مفتوح): كان يُنشئ شفتاً جديداً.
