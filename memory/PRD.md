...
- الآلية: `BroadcastChannel("maestro-egp-tabs-v1")` أساسي + `localStorage` heartbeat كل 1.5 ثانية (تتقادم بعد 5 ثوانٍ).
  - عند فتح تبويب: PING على القناة. لو ورد PONG خلال 400ms → التبويب الجديد **يُحظر**. لو لا → يستلم الملكية ويكتب heartbeat.
  - عند إغلاق التبويب المالك: `beforeunload/pagehide` يمسح `active_tab` ليسمح لتبويب جديد بالفتح فوراً.
  - زر **«استخدام هنا وإغلاق النافذة الأخرى»**: يبثّ `TAKEOVER` فيفقد التبويب القديم ملكيته ويظهر عنده نفس شاشة الحظر، ثم يُعاد تحميل الحالي كمالك جديد.
  - زر **«إغلاق هذا التبويب»**: `window.close()` (يعمل لو التبويب فُتح عبر window.open) وإلا يوجّه لـ `about:blank`.
- **test-ids:** `single-tab-block-overlay`, `single-tab-takeover-btn`, `single-tab-close-btn`.
- **التحقق (Playwright):** تبويب 1 يفتح، ثم كل تبويب لاحق (`/menu`, `/login`, `/track/xxx`, `/driver-app`) يعرض شاشة الحظر ✅.
...
- **الإصلاح:** جُعل هذا الـ GET **عاماً (بلا مصادقة)** ويُرجع بيانات العلامة/التواصل كاملة للجميع — لأنها بيانات تسويق/تواصل عامة مقصودة (صفحة بيع النظام). لم يُعدّل كود الأزرار في SystemContact.js إطلاقاً.
- **ملاحظة على #7:** التراجع مقصود لهذا الحقل تحديداً؛ البيانات معروضة أصلاً على صفحة عامة فلا معنى لإخفائها. بقية إصلاحات الأمان (customer/menu، تكلفة المنتج، payment-settings/supplier-dues/callcenter، حارس الأسعار، المفتاح السري) سليمة (regression 100%).
...
**Frontend (`Dashboard.js`):** شارة كهرمانية `data-testid="merged-shifts-badge"` تظهر داخل نافذة إغلاق الصندوق فقط عند `merged_shifts_count > 0`، تعرض العدد + توضيح عربي أن مبيعات الورديات المكررة وُحّدت دون فقدان بيانات. أُضيفت أيقونة `Layers`.
**التحقق:** 6/6 unit (test_closing_dedupe.py) + فحص حيّ (وردتان مفتوحتان لنفس الكاشير/اليوم → consolidated=2، merged_count=1، يختار الأقدم) + الواجهة تُترجم بلا أخطاء. **⚠️ يتطلب Save to GitHub + Deploy للإنتاج.**



## FIX P0 (يوليو 2026) — تطابق نافذة الإغلاق مع التقرير: دمج الورديات المفتوحة المتعددة عند الملخص/الإغلاق ✅ (iter270)
**المشكلة:** بعد إصلاح تكرار التقرير، نافذة «إغلاق الصندوق» في نقاط البيع كانت تعرض 20,000 بينما التقرير 2,266,500. السبب: باغ الفتح المزدوج ترك ورديتين مفتوحتين لنفس الكاشير (09:00 فيها مبيعات اليوم، و15:41 فيها 20,000)، ومسارا `cash-register/summary` و`cash-register/close` كانا يختاران وردية مفتوحة **عشوائية** (find_one) ويحسبان الطلبات من بداية تلك الوردية فقط.
**الحل:** دالة `_resolve_open_shift_for_close(db, shift_query, tenant_id)` (routes/shifts_routes.py) تختار **الوردية الأقدم بدءاً** وتجمع كل الورديات المفتوحة لنفس (الفرع + الكاشير + اليوم). يستخدمها كلا المسارين: `shift_start = earliest_start`، وكل استعلامات الطلبات `{'shift_id': {'$in': consolidated_ids}}` + غير المرتبطة منذ الأقدم → فتُحسب مبيعات اليوم كاملة (2,266,500) مطابقةً للتقرير. عند الإغلاق تُغلق الوردية الأقدم بالمبلغ الصحيح وتُوسم الأخرى `status='merged', merged_into=<anchor>` (لا حذف، أثر محفوظ). الحالة العادية (وردية واحدة) بلا تغيير.
**التحقق (iter270 = 100% باك+واجهة، 6/6 + 4/4 unit):** الملخص والنافذة يعرضان 2,266,500؛ الإغلاق يسجّل 2,266,500 ويدمج المكررة؛ يبقى إغلاق واحد. خادمي عبر /api (يظهر فور النشر بلا كاش).
**Backlog:** حفظ notes الأصلية عند الإغلاق (تُمسح حالياً)؛ إعادة تسمية `_dedupe_closed_cashier_shifts`→`_dedupe_cashier_shifts`.

## FIX P0 (يوليو 2026) — التصحيح النهائي: الدمج يشمل الورديات المفتوحة (نشط) على الخادم ✅ (iter269)
**الاكتشاف الحاسم:** الوردتان المكررتان لدى المستخدم **مفتوحتان (status='open'، شارة «نشط»)** تُجلبان عبر `/api/shifts?status=open&cashiers_only=true`، ودمج iter268 كان يعالج المغلقة فقط — لذلك لم يتغيّر شيء.
**التصحيح:** في `_dedupe_closed_cashier_shifts` (routes/shifts_routes.py) أصبح `candidates = list(shifts)` بلا فلترة حالة → يدمج المفتوحة والمغلقة معاً (يُستدعى فقط عند `cashiers_only=true`). يبقى مرتكزاً على مبيعات الطلبات الحقيقية للفرع/اليوم (يعالج نسب الطلبات لاسم مُشغّل مختلف)، ±1% للحفاظ على الورديات المشروعة، try/except آمن، وبلا حذف من القاعدة.
**التحقق (iter269 = 100% باك+واجهة، 6/6 + 4/4 unit):** `/api/shifts?status=open&cashiers_only=true` يُعيد محمد صبحي مرة واحدة=2,266,500 رغم أن الطلبات باسم معتز مهنا؛ بلا cashiers_only يُعيد الصفّين؛ القاعدة تحتفظ بالوثيقتين. يظهر فوراً بعد نشر الخادم (طلبات /api لا يخزّنها الـ SW).
**Backlog:** إعادة تسمية الدالة إلى `_dedupe_cashier_shifts`؛ استخراج المنطق لوحدة موحّدة.

## FIX P0 (يوليو 2026) — الحل الحاسم على الخادم: دمج الورديات المغلقة المكررة في /api/shifts ✅ (iter268)
**لماذا فشلت المحاولات السابقة على الإنتاج:** (أ) الطلبات منسوبة لاسم مُشغّل مختلف (معتز مهنا) بينما الوردية باسم محمد صبحي → فشلت المطابقة بالاسم في الواجهة؛ (ب) الـ PWA Service Worker يخدم حزمة JS قديمة من الكاش فلا تظهر تعديلات الواجهة رغم النشر.
**الحل الحاسم (على الخادم، يتجاوز الكاش):** دالة `_dedupe_closed_cashier_shifts(db, shifts)` في `routes/shifts_routes.py` تُستدعى داخل `GET /api/shifts` **فقط عند `cashiers_only=true`**. تُجمّع الورديات المغلقة حسب (الفرع + اسم الكاشير + اليوم)، وللمجموعات المكررة تحسب **مبيعات الطلبات الحقيقية للفرع/اليوم** (branch_day_truth عند وجود كاشير واحد بالفرع/اليوم — يعالج اختلاف اسم المُشغّل، وإلا cashier_day_truth). إن كان مجموع الصفوف ≈ الحقيقة (±1%) تُبقي الكل (ورديات مشروعة)، وإلا تُبقي الأقرب للحقيقة وتستبعد المضخّم. مغلّفة بـ try/except وتُبقي الكل عند غياب المرجع (آمنة). **لا تحذف من القاعدة** — تصفّي الاستجابة فقط. وبما أن الـ SW لا يخزّن `/api`، يظهر الأثر فور نشر الخادم حتى مع واجهة قديمة.
- طبقة ثانية احتياطية في الواجهة (`Reports.js` dedupeClosings حسب الفرع+الكاشير مع مرجع إجمالي الفرع) + backend dedupe لسجلات `cash_register_closings`.
- تحسين نشر: أضيف شريط «تحديث الآن» آمن (index.js + sw-offline.js v38, updateViaCache:'none') لتفعيل النسخ الجديدة فوراً مستقبلاً.
**التحقق (iter268 = 100% باك+واجهة، 6/6 unit):** `/api/shifts?cashiers_only=true` يُعيد محمد صبحي مرة واحدة=2,266,500 رغم أن الطلبات باسم معتز مهنا؛ نفس الاستعلام بلا cashiers_only يُعيد الصفّين (النطاق صحيح)؛ القاعدة تحتفظ بالوثيقتين (لا حذف).
**⚠️ يتطلب Save to GitHub + Deploy** — وبما أنه خادمي، يظهر فور اكتمال النشر بالضغط على «تحديث» (دون انتظار كاش المتصفح).



## FIX P0 (يوليو 2026) — الحل النهائي المُحصّن: مطابقة إغلاق الصندوق للمبيعات + سبب عدم ظهور الإصلاح على الإنتاج ✅ (iter267)
**المشكلة:** «إغلاق الصندوق» يعرض صفّين لنفس الكاشير محمد صبحي/نفس اليوم: صحيح 2,266,500/143 (يطابق المبيعات) ومضخّم 2,286,500/145 (superset). الكاشير = محمد صبحي (المالك يعمل بوضع الانتحال/المعاينة كـ محمد صبحي).
**الحل النهائي (مُحصّن، مرتكز على الطلبات، بلا حذف):**
- Backend (`server.py`): `get_cash_register_closing_report` يُعيد `by_cashier`, `by_branch`, `summary.total_sales` (+ `true_by_cashier_day`/`true_by_branch_day`) — كلها من الطلبات الفعلية بلا تكرار.
- Frontend (`Reports.js` `dedupeClosings(list, ctx)`): التجميع حسب **(الفرع + الكاشير) بلا مكوّن اليوم** (لتفادي اختلاف حساب اليوم بين الواجهة والخادم). المرجع = مبيعات الكاشير من `by_cashier`؛ وعند تعذّر مطابقة اسم/معرّف الكاشير يُستخدَم **إجمالي مبيعات الفرع** كمرجع (عند وجود كاشير واحد بالفرع). إن كان مجموع الصفوف ≈ المرجع → ورديات مشروعة تُبقى؛ وإلا مكررات → يُبقى الأقرب للمبيعات ويُستبعَد المضخّم.
- التحقق: testing_agent iter267 = 100% (صفّ واحد 2,266,500/143، المضخّم مُستبعَد، لا حذف من القاعدة) + 6/6 unit.
**🔴 سبب أن الإصلاح لم يظهر رغم النشر (deploy #452):** الـ PWA Service Worker (`public/sw-offline.js`) يُعطّل `skipWaiting()` عمداً + `index.js` يسجّله بـ `updateViaCache:'all'` → بعد النشر تبقى النسخة الجديدة «waiting» وتُخدَم النسخة القديمة من الكاش حتى تُغلق **كل** تبويبات التطبيق. الحل: بعد النشر أغلق كل التبويبات/التطبيق تماماً ثم افتحه (أو امسح بيانات الموقع مرة واحدة). تحسين مقترح: إضافة شريط «نسخة جديدة متوفرة — تحديث» آمن.



## FIX P0 (يوليو 2026) — الحل النهائي: مطابقة إغلاق الصندوق للمبيعات عبر الحقيقة المرجعية من الطلبات ✅ تم التحقق (iter266)
**المشكلة الحقيقية (من صور المستخدم):** تبويب «إغلاق الصندوق» يعرض صفّين لنفس الكاشير (محمد صبحي) بنفس اليوم: صحيح 2,266,500/143 (يطابق تبويب المبيعات) ومكرر مضخّم 2,286,500/145. المكرر ليس مطابقاً (superset يعدّ نفس الطلبات + 2 إضافية) فلم تنفع قاعدة التداخل/البصمة السابقة. مصدر بيانات المستخدم = وثائق `shifts` (الإغلاق عبر `close_cash_register` لا يكتب في `cash_register_closings`)، لذا الـ dedupe على المجموعة القديمة لم يمسّها.
**الحل النهائي (مرتكز على الطلبات، آمن لكل المستأجرين، بلا حذف):**
- Backend (`server.py` `get_cash_register_closing_report`): يُعيد الآن `true_by_cashier_day` = مبيعات كل كاشير في كل يوم تشغيلي محسوبة من الطلبات الفعلية (بلا تكرار)، بمفاتيح `id:<cashier_id>|<YYYY-MM-DD>` و`name:<name_lc>|<day>`.
- Frontend (`Reports.js` `dedupeClosings(list, truthMap)`): يُجمّع الإغلاقات حسب (الفرع + الكاشير + اليوم). إذا كان **مجموع الصفوف ≈ الحقيقة** (±0.5%) → ورديات منفصلة مشروعة (صباح/مساء) تُبقى كلها. وإلا فهي **مكررات** → يُبقى الصف الأقرب للمبيعات الحقيقية ويُستبعد المضخّم. عند غياب الحقيقة → احتياطي متحفّظ (تداخل زمني أو بصمة متطابقة). لا حذف من القاعدة إطلاقاً.
**التحقق (testing_agent iter266 = 100% + 6/6 unit):** لمحمد صبحي 2026-07-02 يظهر صفّ واحد 2,266,500/143 مطابق للمبيعات، والمكرر 2,286,500/145 اختفى تلقائياً، والوثيقتان ما زالتا في القاعدة (استبعاد على مستوى العرض فقط).
**⚠️ حرِج:** بيانات المستخدم على الإنتاج (maestroegp.com) — يجب **«Save to GitHub» ثم Deploy** ليظهر الإصلاح؛ لهذا لم يرَ أثر الإصلاحات السابقة.
...
---
## تحديث 6 يوليو 2026 — نظام الأمان الصارم (2FA + جهاز موثوق + حظر دائم)

### ما تم تنفيذه واختباره
- **مصادقة ثنائية بنموذج "جهاز موثوق":** أول دخول/جهاز جديد يتطلب رمز تحقق (OTP). بعد التحقق يُوثَّق الجهاز فلا يُطلب الرمز مجدداً منه.
  - القنوات: المالك=بريد (owner@maestroegp.com + Moataz0111@gmail.com)؛ الموظف بهاتف+Twilio=واتساب؛ غير ذلك=بريد الحساب؛ السائق=واتساب/SMS.
  - عند تعذّر الإرسال (بريد/Twilio غير مهيأين) → وضع "معلّق" يُظهر الرمز للمالك (pending-2fa-codes) وفي الاستجابة (dev_code) للتشغيل، ولا يظهر في الإنتاج.
- **مفتاح تفعيل رئيسي بيد المالك** (security-2fa-toggle): 2FA معطّل افتراضياً. عند التنشيط يُخرَج جميع المستخدمين/السائقين فوراً (حذف trusted_devices + driver_tokens + sessions_valid_after) لإجبارهم على التحقق. إبطال الجلسات عبر iat في JWT.
- **حظر IP دائم** بعد 5 محاولات فاشلة (يمنع كل الأجهزة على الشبكة). **عنوان المالك يُحفظ ولا يُحظر أبداً** (owner_trusted_ips).
- **لوحة أمان المالك:** عرض/رفع حظر الـIP، الأجهزة الموثوقة + إلغاؤها، رموز التحقق المعلّقة، حالة التكاملات، تقرير الجاهزية (بيانات ناقصة)، زر «تنشيط التحقق الآن»، وزر حذف البيانات الوهمية.
- **حذف البيانات الوهمية** (purge-dummy-data) بأنماط آمنة لا تمسّ الأسماء الحقيقية.
- **تقنيع الوِجهة:** البريد ***@domain؛ الهاتف كل الأرقام مقنّعة عدا آخر رقمين.
- **حقل اختيار البلد (E.164)** في كل نماذج الإنشاء/التعديل: السائقين، العملاء، الموظفين، المستخدمين (مع حقل هاتف جديد للمستخدم)، المستأجرين.
- التكامل: Twilio Verify (SMS+واتساب). Account SID مضبوط؛ ينقص Auth Token + Verify Service SID من المالك لتفعيل SMS/واتساب فعلياً. البريد يعمل في الإنتاج.

### الاختبار
- iteration_274: 11/11 (2FA/حظر/purge/نقاط المالك). iteration_275: 10/11 ثم أُصلح حقل phone في UserResponse وتم التحقق يدوياً (create/GET/PUT/delete).

### متبقٍ / لاحقاً
- (P0 للمالك) إدخال Twilio Auth Token + Verify Service SID لتفعيل واتساب/SMS فعلياً.
- (P1) بوابة تحقق العميل عند أول طلب من جهاز جديد (OTP هاتف) — لم تُنفَّذ بعد في واجهة الطلب العام.
- (P1) واجهة السائق: نافذة إدخال رمز التحقق في DriverApp (الباك جاهز عبر /driver/login/verify-2fa).
- (P1) موافقة المدير على فروقات النقد الكبيرة (409 → نافذة تجاوز).
- (P2) نقل التوكن إلى HttpOnly cookies؛ سياسة كلمات مرور أقوى.
- (P3) تقسيم server.py (>25k سطر) إلى وحدات routes.

### تحديث لاحق (6 يوليو): شاشة رمز التحقق للسائق مكتملة
- DriverApp: عند الدخول من جهاز جديد (حين التفعيل) تظهر خطوة إدخال الرمز (واتساب/SMS) مع تقنيع الرقم (آخر رقمين)، وزر «تأكيد الرمز» عبر /driver/login/verify-2fa. مختبَر بنجاح.
- تبقّى: مفاتيح Twilio (Auth Token + Verify SID)، بوابة تحقق العميل عند أول طلب، رسالة ترحيب واتساب، موافقة المدير على فروقات النقد.


### تحديث حرِج (6 يوليو 2026): عزل بيانات كل وردية/مستخدم + إلغاء الدمج + إخفاء رمز التحقق
**المشكلة (بلاغ المستخدم):** أرقام المصاريف والصندوق لا تتطابق بين الشاشات؛ بطاقة وردية في التقارير تُظهر مصاريف 11,000 بينما تفاصيلها تجمع 113,000 (كل مصاريف الفرع)؛ إغلاق كاشير آخر يُظهر 108,000. السبب الجذري: المصاريف بلا ربط بوردية — كانت تُنسب بـ (الفرع + الوقت) فتختلط مصاريف الكاشيرين. مطلب المستخدم: كل بيانات/عمل مستخدم تخصّه وحده، ولا دمج إطلاقاً، ودقّة تامة.
**الإصلاح (مختبَر backend 10/10 = 100%):**
- قاعدة إسناد واحدة معتمدة `shift_expense_query` (routes/shared.py): المصروف يخصّ الوردية إذا shift_id=معرّفها، أو (سجل قديم) نفس الفرع+اليوم التشغيلي+منشئ المصروف. مطبّقة في: ملخص الصندوق، إغلاق الصندوق، تفاصيل مصاريف التقرير (Reports.js).
- المصاريف الجديدة تُختم بـ shift_id + cashier_id عند الإنشاء (POST /api/expenses).
- إلغاء الدمج نهائياً: _resolve_open_shift_for_close لم تعد تجمع الورديات؛ كل وردية تُغلق مستقلة، وأُزيل وسم "merged".
- ترحيل بدء التشغيل يعيد احتساب total_expenses للورديات المغلقة بالقاعدة الجديدة (flag expenses_shift_scope_fix) — يصحّح البيانات التاريخية.
- موافقة المدير على فرق نقدي كبير: نافذة manager-approval-dialog في Dashboard.js عند 409 CASH_DISCREPANCY_APPROVAL_REQUIRED؛ الخلفية تتحقق فعلياً من بيانات المدير قبل السماح + حدّ مطلق 50,000 د.ع (حتى لو المبيعات صفر).
- إخفاء رمز التحقق (dev_code) نهائياً من كل استجابات الـAPI والواجهة (SuperAdmin/Login/DriverApp).
**تبقّى (لم يُنفّذ بعد):** بوابة تحقق العميل عند أول طلب، التحقق من ربط واتساب المجاني (يتطلب مسح QR بجهاز فعلي)، رسالة ترحيب واتساب، مفاتيح Twilio.


### ميزتان جديدتان (6 يوليو 2026): تحقق أول طلب للعميل + ربط واتساب برقم الهاتف
- **ربط واتساب برقم الهاتف (Pairing Code)** بديلاً عن مسح QR: المالك يُدخل رقمه في لوحة الأمان ويحصل على رمز XXXX-XXXX يُدخله في واتساب (الأجهزة المرتبطة ← ربط جهاز ← ربط برقم الهاتف). Endpoint: POST /api/super-admin/whatsapp/pair. (مختبَر — يعمل عند اتصال wa_service).
- **بوابة تحقق أول طلب للعميل**: عند تفعيل 2FA، يجب توثيق هاتف العميل عبر واتساب قبل أول طلب (endpoints request-otp/verify-otp؛ نافذة OTP في CustomerMenu؛ create_customer_order يرجع 403 CUSTOMER_PHONE_VERIFICATION_REQUIRED). التوثيق يُخزَّن في verified_customer_phones ويُعتمد لكل الطلبات اللاحقة. (مختبَر backend 5/6، تخطٍّ واحد لقيود preview).
- ⚠️ خطر تشغيلي موثّق: تفعيل 2FA العام دون قناة تسليم OTP فعّالة قد يقفل الوصول (الرمز لا يُعرض إطلاقاً).
**تبقّى (P2):** رسالة ترحيب واتساب بكود خصم للعملاء الجدد الموثّقين، نقل التوكن إلى HttpOnly cookies، سياسة كلمات مرور أقوى، تقسيم server.py.

### ميزة مكتملة (6 يوليو 2026): كوبون ترحيب شخصي باسم الزبون بموافقة المالك (iter279 — 100%)
**المطلوب:** عند أول طلب لزبون جديد → إشعار نظام + واتساب للمالك؛ الموافقة تفتح نموذجاً (مثل نموذج الكوبون العادي) يُدخل فيه المالك عدد مرات الاستخدام والفروع المسموحة؛ يُنشأ كوبون باسم الزبون حصرياً ويُضاف تلقائياً للكوبونات؛ يُرسل للزبون واتساب احترافي (الكود، عدد المرات، الفروع، الصلاحية).
**التنفيذ:**
- Backend: GET /api/welcome-approvals (قائمة المعلّقين، أدمن/مدير فقط)؛ grant-welcome-discount يقبل payload (usage_limit, branch_ids, discount_type/value, min_order, valid_days) وينشئ كوبوناً شخصياً (customer_name=اسم الزبون، usage_limit=usage_per_customer، branch_ids) ويعلّم الإشعار مقروءاً؛ create_customer_order عند أول طلب يُدخل إشعار welcome_approval ويرسل واتساب لرقم owner_phone للمستأجر (fire-and-forget).
- Frontend: WelcomeGrantDialog.js (نموذج الموافقة المشترك)، WelcomeApprovalsBell.js (جرس هدية في هيدر Dashboard مع عداد)، تبويب "طلبات الترحيب" في /coupons مع بطاقات موافقة، زر Settings>العملاء يفتح نفس النموذج.
- الاختبار: iteration_279 — باكند 6/6، فرونتند 100%. الواتساب يفشل بأمان (not_connected) في المعاينة حتى ربط الرقم الفعلي.
- أُصلح أيضاً: تلف نهاية server.py (كتلة مكررة تالفة كانت ستمنع الإقلاع).
**ملاحظة UX معلّقة (P3):** تنبيهات ManagementOrderAlerts تتراكب أحياناً فوق أزرار الأعمدة اليسرى في /coupons و/settings (موجودة منذ iter278).

### إصلاح حرِج (7 يوليو 2026): انزياح تاريخ الشفتات يوماً للخلف (UTC vs توقيت العراق) — iter280 مختبر 100%
**البلاغ:** شفت يوم 6 يظهر تحت يوم 5؛ شفت اليوم مفقود من فلتر "اليوم" بينما مبيعاته ظاهرة (لقطات من النسخة المنشورة).
**الجذر:** الواجهة كانت تحسب تواريخ الفلاتر بـ `new Date().toISOString().split('T')[0]` (تاريخ UTC) — لمستخدم العراق (+3) بعد 21:00 UTC يُرجع الأمس؛ وفي Reports.js computeDateRange كان يبني منتصف الليل المحلي ثم ينسقه UTC → ناقص يوم دائماً.
**الإصلاح:** دالة موحدة `/app/frontend/src/utils/date.js::localDate()` (مكونات التاريخ المحلي) واستبدال كل المواضع في: Reports, Expenses, Orders, Dashboard, Delivery, PurchasesPage, OwnerWallet, BreakEvenReport. الخلفية كانت صحيحة أصلاً (iraq_date_from_utc +3h) + auto_heal يعيد حساب business_date عند كل إقلاع.
**الاختبار:** iteration_280 في نافذة الخلل بالضبط (2 فجراً عراق/22:53 UTC): باكند 5/5 (شفت جديد business_date=07-07، يظهر بفلتر 07-07، لا يظهر بـ07-06) + واجهة بمنطقة Asia/Baghdad تعرض 2026-07-07 صحيحاً وصفر أخطاء كونسول على 6 صفحات.
**مهم:** النسخة المنشورة قديمة — يجب إعادة النشر (خطأ البناء emergentintegrations في requirements.txt أُصلح بنفس الجلسة + deployment check = PASS).

### ميزتان مكتملتان (7 يوليو 2026): فحص السلامة + عرض "حسب الكاشير" (iter281 + iter282 — 100%)
1) **business_date على سجلات الإغلاق:** يُنسخ من يوم الشفت عند الإغلاق + backfill تلقائي عند الإقلاع (8 سجلات صُححت) + فلترة تقرير الإغلاقات بـ business_date أولاً → تطابق تام بين بطاقة الشفت وتقرير إغلاق الصندوق.
2) **فحص السلامة (Integrity Check):** GET /api/integrity/shifts-check يقارن (قراءة فقط) مجاميع كل شفت مغلق مع الطلبات/المصاريف الفعلية وسجل إغلاقه؛ زر "فحص السلامة" في التقارير ← إغلاق الصندوق مع لوحة نتائج؛ فحص تلقائي عند كل إقلاع لليومين الأخيرين مع إشعار نظام (integrity_alert مع dedupe) + واتساب للمالك عند عدم التطابق. اختبار iter281: باكند 7/7 + واجهة 100%.
3) **عرض "حسب الكاشير":** توغل ثالث في تبويب إغلاق الصندوق يجمع إغلاقات الكاشير الواحد (عرض فقط — بلا دمج بيانات) ويظهر إجمالي مبيعاته (طلب المستخدم: محمد صبحي 2,114,750) مع توسيع يعرض كل شفت منفرداً. اختبار iter282: واجهة 100%.
**قاعدة ثابتة من المستخدم:** لا تعديل على قواعد المحاسبة — الشفت منسوب ليوم فتحه ويغلق بأي وقت، لا دمج شفتات في البيانات إطلاقاً.

### تحديث (7 يوليو 2026): أتمتة كاملة لفحص السلامة — بلا أزرار (iter283 + iter284 — 100%)
**مطلب المستخدم الصريح:** "منظومة حسابية لا يجب التدخل اليدوي بها — لا أريد أزرار".
- حُذف زر "فحص السلامة" — الفحص يعمل تلقائياً في التقارير عند تحميل الإغلاقات وأي تغيير للفترة/الفرع (useEffect على closingsHistory).
- مجدول خلفي كل ساعة (start_integrity_scheduler) يفحص اليومين الأخيرين لكل المستأجرين + إشعار وواتساب للمالك (deduped).
- فحص فوري تلقائي بعد كل إغلاق شفت (async task في close endpoint).
- باغ حرج اكتشفه وكيل الاختبار وأُصلح: branch_id='all' كان يُرجع 0 نتائج → "سليم 100%" زوراً. أُصلح من الجهتين (الواجهة تتجاهل 'all'، والخادم يعامل all/*/'' كبلا فلتر). retest iter284: النتائج الحقيقية 7/5/2 تظهر تلقائياً.

### إصلاحات جوهرية (7 يوليو 2026): قاعدة اليوم التشغيلي + فتح الشفت بأول طلب + منع خلط الكاشيرية (iter285 + iter286 — 100%)
**قواعد المستخدم النهائية (لا تُخالف):**
1. **ساعة بداية اليوم التشغيلي (افتراضي 6 صباحاً، قابلة للتخصيص عبر tenant.business_day_start_hour):** أي شفت يُفتح قبلها (فجراً) يُنسب لليوم السابق — الطلبات ترث يوم الشفت — لا يُقسّم ليل المطعم على يومين أبداً. الدوال: iraq_business_date_from_utc + get_business_day_start_hour في shared.py وserver.py. auto_heal يعيد حساب كل الشفتات ويزامن الطلبات عند كل إقلاع (تلقائي، بلا أزرار).
2. **الشفت لا يُفتح عند تسجيل الدخول — يُفتح تلقائياً عند حفظ أول طلب** (أُزيل الفتح التلقائي من AuthContext/Dashboard/POS mount؛ الباكند ينشئ الشفت lazily في POST /orders؛ POS يلتقط الشفت بعد أول طلب).
3. **منع الخلط بين الكاشيرية (حرج — iter285):** كاشير بلا شفت لا يلتقط شفت كاشير آخر في الفرع أبداً — يُنشأ له شفته الخاص. الالتقاط البرانشي للمالك/المدير فقط. تأكيد iter286: 11/11.
**ملاحظة معلقة (MEDIUM):** واجهة تسجيل دخول الكاشير قد تُحجب صامتاً ببوابة الجهاز الموثوق — يلزم إظهار رسالة توضيحية على /login.
**شكوى المستخدم:** وُجّه رسمياً إلى support@emergent.sh مع Job ID لطلب مراجعة الكريدت والتعويض.

## 2026-07-07 — حالة المهام بعد هذه الجلسة
**أُنجز (testing_agent iter100 = 100%):**
- ✅ (P0) تقسيم server.py: 26,082 → 16,861 سطر (13 وحدة routes جديدة).
- ✅ (P1) كوكيز HttpOnly (هجين — Bearer أو كوكي؛ localStorage باقٍ للوضع Offline).
- ✅ (P2) سياسة كلمات مرور قوية (≥8 + حرف + رقم) على إنشاء/تغيير كلمات المرور.
- ✅ (P2) رسالة "جهاز غير موثوق" في نافذة 2FA (untrusted-device-notice).
- ✅ (P3) إحصائيات كوبونات الترحيب (endpoint + لوحة في الإعدادات/العملاء).
- ✅ (P3) الطباعة التلقائية عند إغلاق الوردية — موجودة مسبقاً (تم التحقق).
- ✅ إصلاح 403 agent-status في الإعدادات + إصلاح seed_expense_attribution_test.py (DB_NAME).

**المتبقي (Backlog):**
- (P2) مواصلة تقسيم server.py (لا يزال ~16.8k سطر) و SuperAdmin.js.
- (P2) نقل تحديد المعدّل من الذاكرة إلى MongoDB/Redis عند التوسع الأفقي.
- (P3) تنبيهات ManagementOrderAlerts تتراكب فوق أزرار في /coupons و/settings.
- (P0 للمالك — إنتاج) إدخال Twilio Auth Token لتفعيل SMS الاحتياطي.
