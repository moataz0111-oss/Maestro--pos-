...
- **Local Agent**: PowerShell script (print_server.ps1 v6.0.0 - Server Polling Architecture)

## Print Architecture (v6.0.0)
- Browser sends print jobs to backend via `POST /api/print-queue`
- PowerShell agent polls `GET /api/print-queue/pending` every 3 seconds with `agent_version=6.0.0&device_id=default`
...
- Order_data support in polling job (kitchen USB receipts built locally)
