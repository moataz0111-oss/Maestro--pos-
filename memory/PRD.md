...
### إصلاحات شاملة (March 29, 2026) ✅

**1. إصلاح خطأ 500 الشامل:**
- `shifts_routes.py`: دالة `_safe_num()` + حماية 5 دوال من قيم null
- `server.py`: دالة `_sn()` + إصلاح **202 مكان** خطير (.get("total", 0) → _sn())
- حماية `shift_start` و `cashier_id` من قيم None

**2. إصلاح منطق احتساب الصندوق:**
- Summary: يحسب حسب وردية الموظف (shift_id → cashier_id fallback)
- Close: نفس المنطق المتطابق

**3. إغلاق اليوم والترحيل (Force Close):**
- الإغلاق الإجباري يضع closing_cash = 0 (short cash)
- يحسب expected_cash والفرق بشكل صحيح
- الإغلاق التلقائي بعد 24 ساعة يعمل بنفس الطريقة

**4. صلاحية إخفاء المبيعات حسب طريقة الدفع:**
- جديد: قسم "المبيعات حسب طريقة الدفع" يُخفى مع hide_cash_expected
