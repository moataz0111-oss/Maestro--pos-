...
async def download_print_agent(request: Request):
    ...
    bat_lines = [
        ...
        '',
        'REM ======================================================',
        'REM   Maestro Print Agent v5.0.0 - Full Clean Install',
        'REM ======================================================',
        '',
        'REM === Request Admin ===',
        'net session >nul 2>&1',
        ...
        ')',
        '',
        'title Maestro Print Agent v5.0.0 - Clean Install',
        'color 0A',
        'echo.',
        'echo  ========================================',
        'echo    Maestro Print Agent v5.0.0',
        ...
        'powershell -NoProfile -Command "try { $existing = Get-ChildItem Cert:\\LocalMachine\\Root | Where-Object {$_.Subject -eq \'CN=MaestroPrintAgent\'}; if($existing) { Write-Host \'    Certificate already exists\' -ForegroundColor Green } else { $cert = New-SelfSignedCertificate -DnsName \'localhost\',\'127.0.0.1\' -CertStoreLocation Cert:\\LocalMachine\\My -FriendlyName \'Maestro Print Agent\' -Subject \'CN=MaestroPrintAgent\' -NotAfter (Get-Date).AddYears(10) -KeyAlgorithm RSA -KeyLength 2048; $thumb = $cert.Thumbprint; Export-Certificate -Cert $cert -FilePath ($env:LOCALAPPDATA+\'\\MaestroPrintAgent\\cert.cer\') -Force | Out-Null; Import-Certificate -FilePath ($env:LOCALAPPDATA+\'\\MaestroPrintAgent\\cert.cer\') -CertStoreLocation Cert:\\LocalMachine\\Root -Confirm:$false | Out-Null; netsh http delete sslcert ipport=0.0.0.0:9443 2>$null; netsh http add sslcert ipport=0.0.0.0:9443 certhash=$thumb appid=\'{d4a1c0e1-0000-0000-0000-000000000001}\' | Out-Null; Write-Host \'    HTTPS certificate created and trusted\' -ForegroundColor Green } } catch { Write-Host \'    WARNING: HTTPS setup failed, using HTTP only\' -ForegroundColor Yellow; Write-Host $_.Exception.Message }"',
        '',
        ...
        '',
        'REM ========================================',
        'REM   STEP 3: DOWNLOAD FRESH v5.0.0',
        'REM ========================================',
        'echo  [3/6] Downloading v5.0.0 (no cache)...',
        ...
        'REM ========================================',
        'echo  [4/6] Starting new agent v5.0.0...',
        ...
        'powershell -NoProfile -Command "try { $r=Invoke-WebRequest -Uri \'http://localhost:9999/status\' -UseBasicParsing -TimeoutSec 10; $j=$r.Content|ConvertFrom-Json; Write-Host (\'  Agent Version: \'+$j.version) -ForegroundColor Green; if($j.version -eq \'5.0.0\'){Write-Host \'  v5.0.0 OK!\' -ForegroundColor Green}else{Write-Host \'  WARNING: Expected 5.0.0 got \'+$j.version -ForegroundColor Red} } catch { Write-Host \'  Agent starting... wait 30 sec and refresh browser\' -ForegroundColor Yellow }"',
        'echo.',
        'echo  ========================================',
        'echo    DONE! Refresh the POS page.',
        'echo    Agent v5.0.0 installed.',
        'echo  ========================================',
        'echo.',
        'pause',
        'exit /b',
