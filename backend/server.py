...
async def download_print_agent(request: Request):
    ...
    bat_lines = [
        ...
        '',
        'REM ======================================================',
        'REM   Maestro Print Agent v5.1.0 - Full Clean Install',
        'REM ======================================================',
        '',
        'REM === Request Admin ===',
        'net session >nul 2>&1',
        ...
        ')',
        '',
        'title Maestro Print Agent v5.1.0 - Clean Install',
        'color 0A',
        'echo.',
        'echo  ========================================',
        'echo    Maestro Print Agent v5.1.0',
        ...
        '',
        'REM Create self-signed SSL certificate for localhost (HTTPS)',
        'echo  [2.5/6] Setting up HTTPS certificate...',
        '',
        'REM Write cert setup script',
        '(echo $ErrorActionPreference = "Stop") > "%D%\\setup-cert.ps1"',
        '(echo try {) >> "%D%\\setup-cert.ps1"',
        '(echo   $existing = Get-ChildItem Cert:\\LocalMachine\\My ^| Where-Object {$_.Subject -eq "CN=MaestroPrintAgent"}) >> "%D%\\setup-cert.ps1"',
        '(echo   if($existing^) {) >> "%D%\\setup-cert.ps1"',
        '(echo     $thumb = $existing[0].Thumbprint) >> "%D%\\setup-cert.ps1"',
        '(echo     Write-Host "    Certificate exists: $thumb" -ForegroundColor Green) >> "%D%\\setup-cert.ps1"',
        '(echo   } else {) >> "%D%\\setup-cert.ps1"',
        '(echo     $cert = New-SelfSignedCertificate -DnsName "localhost","127.0.0.1" -CertStoreLocation "Cert:\\LocalMachine\\My" -FriendlyName "Maestro Print Agent" -Subject "CN=MaestroPrintAgent" -NotAfter (Get-Date^).AddYears(10^) -KeyAlgorithm RSA -KeyLength 2048 -KeyExportPolicy Exportable) >> "%D%\\setup-cert.ps1"',
        '(echo     $thumb = $cert.Thumbprint) >> "%D%\\setup-cert.ps1"',
        '(echo     Write-Host "    Certificate created: $thumb" -ForegroundColor Green) >> "%D%\\setup-cert.ps1"',
        '(echo     $certPath = $env:LOCALAPPDATA + "\\MaestroPrintAgent\\cert.cer") >> "%D%\\setup-cert.ps1"',
        '(echo     Export-Certificate -Cert $cert -FilePath $certPath -Force ^| Out-Null) >> "%D%\\setup-cert.ps1"',
        '(echo     Import-Certificate -FilePath $certPath -CertStoreLocation "Cert:\\LocalMachine\\Root" -Confirm:$false ^| Out-Null) >> "%D%\\setup-cert.ps1"',
        '(echo     Write-Host "    Certificate trusted" -ForegroundColor Green) >> "%D%\\setup-cert.ps1"',
        '(echo   }) >> "%D%\\setup-cert.ps1"',
        '(echo   netsh http delete sslcert ipport=0.0.0.0:9443 2^>$null) >> "%D%\\setup-cert.ps1"',
        '(echo   $appId = "{d4a1c0e1-0000-0000-0000-000000000001}") >> "%D%\\setup-cert.ps1"',
        '(echo   $result = netsh http add sslcert ipport=0.0.0.0:9443 certhash=$thumb appid=$appId 2^>&1) >> "%D%\\setup-cert.ps1"',
        '(echo   Write-Host "    SSL bound to port 9443: $result" -ForegroundColor Green) >> "%D%\\setup-cert.ps1"',
        '(echo } catch {) >> "%D%\\setup-cert.ps1"',
        '(echo   Write-Host "    HTTPS setup error: $_" -ForegroundColor Red) >> "%D%\\setup-cert.ps1"',
        '(echo }) >> "%D%\\setup-cert.ps1"',
        '',
        'powershell -ExecutionPolicy Bypass -NoProfile -File "%D%\\setup-cert.ps1"',
        '',
        ...
        '',
        'REM ========================================',
        'REM   STEP 3: DOWNLOAD FRESH v5.1.0',
        'REM ========================================',
        'echo  [3/6] Downloading v5.1.0 (no cache)...',
        ...
        'REM ========================================',
        'echo  [4/6] Starting new agent v5.1.0...',
        ...
        'powershell -NoProfile -Command "try { $r=Invoke-WebRequest -Uri \'http://localhost:9999/status\' -UseBasicParsing -TimeoutSec 10; $j=$r.Content|ConvertFrom-Json; Write-Host (\'  Agent Version: \'+$j.version) -ForegroundColor Green; if($j.version -eq \'5.1.0\'){Write-Host \'  v5.1.0 OK!\' -ForegroundColor Green}else{Write-Host \'  WARNING: Expected 5.1.0 got \'+$j.version -ForegroundColor Red} } catch { Write-Host \'  Agent starting... wait 30 sec and refresh browser\' -ForegroundColor Yellow }"',
        'echo.',
        'echo  ========================================',
        'echo    DONE! Refresh the POS page.',
        'echo    Agent v5.1.0 installed.',
        'echo  ========================================',
        'echo.',
        'pause',
        'exit /b',
...
async def get_menu_link(request: Request, current_user: dict = Depends(get_current_user)):
    ...
    if origin:
        ...
        base_url = os.environ.get('REACT_APP_BACKEND_URL', 'https://blind-count-pos.preview.emergentagent.com')
