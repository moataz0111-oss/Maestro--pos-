...
"$(Get-Date) - Agent v2.6.0 starting..." | Out-File $agentLog
...
try {
    ...
    $listener.Start()
    "$(Get-Date) - HttpListener started on port 9999 (v2.6.0)" | Out-File $agentLog -Append
    ...
    while ($listener.IsListening) {
        ...
        if ($path -eq '/status') {
            $jsonOut = '{"status":"running","version":"2.6.0","agent":"Maestro Print Agent","usb_support":true,"zk_support":true}'
        }
