...
"$(Get-Date) - Agent v3.9.1 starting..." | Out-File $agentLog
...
while ($restartCount -lt $maxRestarts) {
  ...
  try {
    ...
    $listener.Start()
    "$(Get-Date) - HttpListener started on port 9999 (v3.9.1) [restart #$restartCount]" | Out-File $agentLog -Append
    ...
    while ($listener.IsListening) {
      try {
        ...
        if ($path -eq '/status') {
            $jsonOut = '{"status":"running","version":"3.9.1","agent":"Maestro Print Agent","usb_support":true,"zk_support":true}'
        }
