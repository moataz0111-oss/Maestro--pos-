...
"$(Get-Date) - Agent v4.2.0 starting..." | Out-File $agentLog
...
while ($restartCount -lt $maxRestarts) {
  ...
  try {
    ...
    $listener.Start()
    "$(Get-Date) - HttpListener started on port 9999 (v4.2.0) [restart #$restartCount]" | Out-File $agentLog -Append
    ...
    while ($listener.IsListening) {
      try {
        ...
        if ($path -eq '/status') {
            $jsonOut = '{"status":"running","version":"4.2.0","agent":"Maestro Print Agent","usb_support":true,"zk_support":true}'
        }
