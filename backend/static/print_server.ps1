...
"$(Get-Date) - Agent v5.1.0 starting..." | Out-File $agentLog
...
while ($restartCount -lt $maxRestarts) {
  ...
  try {
    ...
    $listener.Start()
    "$(Get-Date) - HttpListener started on port 9999 (v5.1.0) [restart #$restartCount]" | Out-File $agentLog -Append
    ...
    while ($listener.IsListening) {
      try {
        ...
        if ($path -eq '/status') {
            $jsonOut = '{"status":"running","version":"5.1.0","agent":"Maestro Print Agent","usb_support":true,"zk_support":true}'
        }
