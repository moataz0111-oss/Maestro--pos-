...
"$(Get-Date) - Agent v3.4.0 starting..." | Out-File $agentLog
...
try {
    ...
    $listener.Start()
    "$(Get-Date) - HttpListener started on port 9999 (v3.4.0)" | Out-File $agentLog -Append
    ...
    while ($listener.IsListening) {
        ...
        if ($path -eq '/status') {
            $jsonOut = '{"status":"running","version":"3.4.0","agent":"Maestro Print Agent","usb_support":true,"zk_support":true}'
        }
