...
def test_build_branded_email_html_has_logo_below_blue_bar():
    ...
    # الشعار عبر CID inline (يعمل في Gmail/Apple/Outlook بلا حجب)
    assert "cid:maestrologo" in html, "الشعار غير موجود (cid:maestrologo مفقود)"
    idx_h2 = html.find("</h2></div>")
    idx_img = html.find("<img")
    idx_body = html.find("<p>مرحباً</p>")
    assert idx_h2 > 0 and idx_img > idx_h2 > 0, "الشعار يجب أن يكون بعد الشريط الأزرق"
    assert idx_body > idx_img > 0, "محتوى الجسم يجب أن يكون بعد الشعار"
...
async def test_send_welcome_email_uses_branded_template():
    ...
    html = captured["html"]
    assert "cid:maestrologo" in html, "الشعار الموحّد (CID) مفقود في بريد الترحيب"
...
async def test_send_shift_report_email_uses_branded_template():
    ...
    html = captured["html"]
    assert "cid:maestrologo" in html, "الشعار (CID) مفقود في تقرير الوردية"
