    ...
    <div className="min-h-screen bg-background" dir="rtl" data-testid="warehouse-page">
      ...
      <main className="max-w-7xl mx-auto p-4 space-y-4">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            {!isWarehouseKeeper && (
              <TabsTrigger value="request-packaging" className="gap-2 px-4 py-2 relative" data-testid="tab-request-packaging">
                ...
                {packagingRequests.filter(r => r.created_by === user?.id && r.status === 'pending').length > 0 && (
                  <Badge className="absolute -top-2 -right-2 bg-blue-500 text-white text-xs px-1.5 py-0.5 rounded-full">
                    {packagingRequests.filter(r => r.created_by === user?.id && r.status === 'pending').length}
                  </Badge>
                )}
              </TabsTrigger>
            )}
          ...
          {/* طلب مواد تغليف - واجهة الفروع */}
          <TabsContent value="request-packaging" className="space-y-4">
            {/* نموذج طلب مواد تغليف */}
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold">{t('طلب مواد تغليف من المخزن')}</h3>
                      <p className="text-sm text-muted-foreground">{t('اختر المواد المطلوبة وأرسل الطلب لأمين المستودع')}</p>
                    </div>
                  </div>
                  <Button
                    onClick={() => setShowRequestHistoryDialog(true)}
                    variant="outline"
                    className="gap-2"
                  >
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 mb-4">
                  {packagingMaterials.map(material => {
                    const inCart = packagingRequestItems.find(i => i.packaging_material_id === material.id);
                    return (
                      <div 
                        key={material.id} 
                        ...
                        onClick={() => !inCart && addItemToPackagingRequest(material)}
                      >
                        <div className="flex items-center justify-between mb-2">
                          <h4 className="font-medium text-sm">{material.name}</h4>
                          {inCart && (
                            <Badge className="bg-blue-500 text-white">{inCart.quantity}</Badge>
                          )}
                        </div>
                        <p className="text-xs text-muted-foreground">{t('المتاح')}: {material.quantity} {material.unit}</p>
                        <p className="text-xs text-muted-foreground">{formatPrice(material.cost_per_unit)}/{material.unit}</p>
                        {inCart && (
                          <div className="flex items-center gap-2 mt-2">
                            <Button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                updatePackagingRequestItemQty(
                                  packagingRequestItems.findIndex(i => i.packaging_material_id === material.id),
                                  Math.max(1, inCart.quantity - 1)
                                );
                              }}
                            >
                ...
                {packagingMaterials.length === 0 && (
                  <div className="text-center py-8 text-muted-foreground">
                    <Box className="h-12 w-12 mx-auto mb-4 opacity-50" />
                    <p>{t('لا توجد مواد تغليف متاحة للطلب')}</p>
                  </div>
                )}
                
                {/* سلة الطلب */}
                {packagingRequestItems.length > 0 && (
                  <Card className="border-blue-500/50 bg-blue-500/5">
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="font-bold flex items-center gap-2">
                          <ShoppingCart className="h-5 w-5 text-blue-500" />
                          {t('سلة الطلب')} ({packagingRequestItems.length} {t('صنف')})
                        </h4>
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => setPackagingRequestItems([])}
                      ...
                      <div className="space-y-2 mb-4">
                        {packagingRequestItems.map((item, idx) => (
                      ...
                      <div className="mb-4">
                        ...
                        <Textarea
                          ...
                          onChange={(e) => setPackagingRequestNotes(e.target.value)}
                          placeholder={t('أضف أي ملاحظات للطلب...')}
                          className="mt-1"
                          rows={2}
                        />
                      </div>
                      ...
                      <Button
                        onClick={handleSubmitPackagingRequest}
                        disabled={submitting}
                        className="w-full bg-blue-500 hover:bg-blue-600"
                        data-testid="submit-packaging-request-btn"
                      >
                        <Send className="h-4 w-4 ml-2" />
                        {submitting ? t('جاري الإرسال...') : t('إرسال الطلب')}
                      </Button>
                    </CardContent>
                  </Card>
                )}
          ...
          <TabsContent value="branch-packaging-inv" className="space-y-4">
            <Card className="border-green-500/30 bg-green-500/5">
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  <Package className="h-8 w-8 text-green-500" />
                  <div>
                    <h3 className="font-bold">{t('مخزون مواد التغليف في فرعك')}</h3>
                    <p className="text-sm text-muted-foreground">{t('المواد المتوفرة حالياً في الفرع')}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                {branchPackagingInventory.length === 0 ? (
                  <div className="text-center py-12 text-muted-foreground">
                    <Package className="h-12 w-12 mx-auto mb-4 opacity-50" />
                    <p>{t('لا توجد مواد تغليف في مخزون الفرع')}</p>
                    <p className="text-sm mt-2">{t('أرسل طلب للمستودع للحصول على مواد التغليف')}</p>
                    <Button
                      onClick={() => setActiveTab('request-packaging')}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {branchPackagingInventory.map(item => (
                      <div key={item.id} className="p-4 bg-muted/30 rounded-lg border hover:border-green-500/50 transition-colors">
                        <div className="flex items-center justify-between mb-3">
                          <h4 className="font-bold">{item.packaging_material_name}</h4>
                          <Badge className={
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            <p className="text-xs text-muted-foreground">{t('الكمية المتبقية')}</p>
                            <p className="font-bold text-lg text-green-600">{item.remaining_quantity}</p>
                          </div>
                          <div>
                            <p className="text-xs text-muted-foreground">{t('المستخدم')}</p>
                            <p className="font-medium text-orange-600">{item.used_quantity || 0}</p>
                          </div>
                          <div>
                            <p className="text-xs text-muted-foreground">{t('الكمية الإجمالية')}</p>
                            <p className="font-medium">{item.quantity}</p>
                          </div>
                          <div>
                            <p className="text-xs text-muted-foreground">{t('القيمة')}</p>
                            <p className="font-medium">{formatPrice(item.total_value || 0)}</p>
                          </div>
                        </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <Card className="bg-green-500/10 border-green-500/30">
                  <CardContent className="p-4 text-center">
                    ...
                    <p className="text-2xl font-bold text-green-600">{branchPackagingInventory.length}</p>
                  </CardContent>
                </Card>
                <Card className="bg-blue-500/10 border-blue-500/30">
                  <CardContent className="p-4 text-center">
                ...
                <Card className="bg-yellow-500/10 border-yellow-500/30">
                  <CardContent className="p-4 text-center">
                    <p className="text-sm text-muted-foreground">{t('أصناف منخفضة')}</p>
                ...
                <Card className="bg-orange-500/10 border-orange-500/30">
                  <CardContent className="p-4 text-center">
                    <p className="text-sm text-muted-foreground">{t('إجمالي المستخدم')}</p>
                    <p className="text-2xl font-bold text-orange-600">
                      {branchPackagingInventory.reduce((sum, i) => sum + (i.used_quantity || 0), 0)}
                    </p>
                  </CardContent>
                </Card>
              </div>
