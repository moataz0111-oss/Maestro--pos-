...
export default function Settings() {
  ...
  const [productForm, setProductForm] = useState({
    name: '', name_en: '', category_id: '', price: '', cost: '', operating_cost: '', packaging_cost: '', image: '', description: '', barcode: '', manufactured_product_id: '', manufactured_links: [], recipe_quantities: [], printer_ids: [], extras: [], packaging_items: []
  });
  ...
  useEffect(() => {
    if (hasRole(['admin', 'general_manager', 'super_admin', 'manager', 'branch_manager'])) {
      fetchData();
  ...
  const fetchRestaurantSettings = async () => {
    try {
      ...
      if (res.data.logo_url) {
        ...
        // إذا كان الـ URL نسبي (يبدأ بـ /)، أضف الـ BACKEND base
  ...
  const handleCreateProduct = async (e) => {
    ...
    try {
      await axios.post(`${API}/products`, {
        ...
        manufactured_product_id: productForm.manufactured_product_id || (productForm.manufactured_links?.[0]?.manufactured_product_id) || null,
        manufactured_links: (productForm.manufactured_links || []).filter(l => l.manufactured_product_id),
      ...
      setProductForm({ name: '', name_en: '', category_id: '', price: '', cost: '', operating_cost: '', packaging_cost: '', image: '', description: '', barcode: '', manufactured_product_id: '', manufactured_links: [], recipe_quantities: [], printer_ids: [], extras: [], packaging_items: [] });
  ...
  const handleEditProduct = (p) => {
    ...
    setEditProductForm({
      ...
      // ⭐ ربط متعدد — إذا كانت البيانات قديمة (single link)، نُحوّلها إلى array
      manufactured_links: (p.manufactured_links && p.manufactured_links.length > 0)
        ? p.manufactured_links
        : (p.manufactured_product_id
            ? [{ manufactured_product_id: p.manufactured_product_id, consumption_qty: Number(p.manufactured_consumption_qty || 1) }]
            : []),
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      await axios.put(`${API}/products/${editProductForm.id}`, {
        ...
        manufactured_product_id: editProductForm.manufactured_product_id || (editProductForm.manufactured_links?.[0]?.manufactured_product_id) || null,
        manufactured_links: (editProductForm.manufactured_links || []).filter(l => l.manufactured_product_id),
  ...
  const renderMfgLinksEditor = ({ form, setForm, formKind = 'add' }) => {
    ...
    const recalcAndSet = (newLinks) => {
      ...
      setForm({
        ...
        // مزامنة الحقل القديم للتوافق
        manufactured_product_id: newLinks[0]?.manufactured_product_id || null,
        manufactured_consumption_qty: Number(newLinks[0]?.consumption_qty || 1),
        cost: total > 0 ? total.toFixed(0) : (form.cost || ''),
      });
    };
    ...
    return (
      <div className="p-3 bg-purple-500/10 border border-purple-500/30 rounded-lg space-y-3" data-testid={`mfg-links-editor-${formKind}`}>
        <Label className="text-foreground font-medium flex items-center gap-2">
          ...
          {t('ربط بمنتجات مُصنّعة (للخصم التلقائي)')}
        </Label>

        {manufacturedProducts.length === 0 ? (
          <p className="text-xs text-amber-600 bg-amber-500/10 p-2 rounded">
            {t('لا توجد منتجات مصنعة حالياً. أضف منتجات من قسم المخزن والتصنيع أولاً')}
          </p>
        ) : (
          <>
            {/* قائمة الروابط الحالية */}
            {links.length === 0 && (
              <p className="text-xs text-muted-foreground">
                {t('لا توجد منتجات مُصنّعة مرتبطة بعد. اضغط على "إضافة منتج مُصنّع" للبدء.')}
              </p>
            )}
            {links.map((lk, idx) => {
              const mp = manufacturedProducts.find(m => m.id === lk.manufactured_product_id);
              ...
              return (
                <div key={idx} className="p-3 bg-background/50 rounded-lg border border-purple-500/30 space-y-2" data-testid={`mfg-link-row-${idx}`}>
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <SelectTrigger className="flex-1 h-9" data-testid={`mfg-link-select-${idx}`}>
                        <SelectValue placeholder={t('اختر المنتج المُصنّع')} />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    <Button
                      ...
                      data-testid={`mfg-link-remove-${idx}`}
                    >
            ...
            {/* إجمالي التكلفة (مجموع كل الروابط) */}
            {links.length > 0 && (
              <div className="p-3 rounded-lg bg-emerald-500/10 border-2 border-emerald-500/40 flex items-center justify-between" data-testid={`mfg-links-total-${formKind}`}>
                <span className="text-sm font-bold text-emerald-700 dark:text-emerald-400">
                  ⭐ {t('إجمالي تكلفة المنتج النهائي')}
                </span>
                <span className="text-lg font-extrabold text-emerald-600 tabular-nums">{formatPrice(totalCost)}</span>
              </div>
            )}
            ...
            <div className="flex gap-2 pt-1">
              ...
              >
                ...
                {links.length === 0 ? t('ربط بمنتج مُصنّع') : t('إضافة منتج مُصنّع آخر')}
              </Button>
            </div>

            <p className="text-xs text-muted-foreground">
              {t('عند البيع: تُخصم كل المنتجات المُصنّعة المربوطة تلقائياً بكمياتها المحددة. التكلفة الإجمالية = مجموع تكاليف جميع المكونات.')}
            </p>
          </>
        )}
      </div>
        ...
        <Tabs defaultValue="appearance" className="space-y-6">
          ...
          <div className="bg-card border rounded-xl p-2 overflow-x-auto">
            <TabsList className="flex flex-wrap gap-2 w-full h-auto bg-transparent p-0">
              ...
              {(hasRole(['admin', 'general_manager', 'super_admin', 'manager', 'branch_manager']) || hasSettingsPermission('settings_customers')) && settingsPermissions.settingsCustomers && (
                <TabsTrigger 
                  value="customers"
                  className="flex-shrink-0 px-4 py-2.5 text-sm font-medium rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
                >
              ...
              {hasRole(['admin', 'general_manager', 'super_admin']) && settingsPermissions.settingsBranches && (
                <TabsTrigger 
                  value="branches"
                  className="flex-shrink-0 px-4 py-2.5 text-sm font-medium rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
                >
              ...
              {(hasRole(['admin', 'general_manager', 'super_admin', 'manager', 'branch_manager']) || hasSettingsPermission('settings_categories')) && settingsPermissions.settingsCategories && (
                <TabsTrigger 
                  value="categories"
                  className="flex-shrink-0 px-4 py-2.5 text-sm font-medium rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
                >
              ...
              {(hasRole(['admin', 'general_manager', 'super_admin', 'manager', 'branch_manager']) || hasSettingsPermission('settings_products')) && settingsPermissions.settingsProducts && (
                <TabsTrigger 
                  value="products"
                  className="flex-shrink-0 px-4 py-2.5 text-sm font-medium rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
                >
              ...
              {(hasRole(['admin', 'general_manager', 'super_admin', 'manager', 'branch_manager']) || hasSettingsPermission('settings_printers')) && settingsPermissions.settingsPrinters && (
                <TabsTrigger 
                  value="printers"
                  className="flex-shrink-0 px-4 py-2.5 text-sm font-medium rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
                >
              ...
              {(hasRole(['admin', 'general_manager', 'super_admin']) || hasSettingsPermission('settings_delivery')) && settingsPermissions.settingsDeliveryCompanies && (
                <TabsTrigger 
                  value="delivery"
                  className="flex-shrink-0 px-4 py-2.5 text-sm font-medium rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
                >
              ...
              {hasRole(['admin', 'general_manager', 'super_admin']) && settingsPermissions.settingsCallCenter && (
                <TabsTrigger 
                  value="callcenter"
                  className="flex-shrink-0 px-4 py-2.5 text-sm font-medium rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
                >
              ...
              {(hasRole(['admin', 'general_manager', 'super_admin']) || hasSettingsPermission('settings_notifications')) && settingsPermissions.settingsNotifications && (
                <TabsTrigger 
                  value="notifications"
                  className="flex-shrink-0 px-4 py-2.5 text-sm font-medium rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
                >
          ...
          {hasRole(['admin', 'general_manager', 'super_admin', 'manager', 'branch_manager']) && (
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        <div>
                          ...
                          <Select
                            ...
                            onValueChange={(value) => setEditCategoryForm({ ...editCategoryForm, icon: value })}
                          >
                            <SelectTrigger className="mt-1">
                              <SelectValue placeholder={t('اختر أيقونة')} />
                            </SelectTrigger>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                      ...
                      <form onSubmit={handleCreateProduct} className="space-y-4">
                        ...
                        {/* اختيار الطابعات للمنتج الجديد - مجمّعة حسب الفرع */}
                        {printers.length > 0 && (
                          <div className="space-y-2">
                            <Label className="text-foreground">{t('وجهة الطباعة (الطابعات)')}</Label>
                            {/* تجميع الطابعات حسب الفرع */}
                            {(() => {
                              const grouped = {};
                              printers.forEach(p => {
                                const branchName = branches?.find(b => b.id === p.branch_id)?.name || t('بدون فرع');
                                if (!grouped[branchName]) grouped[branchName] = [];
                                grouped[branchName].push(p);
                              });
                              return Object.entries(grouped).map(([branchName, branchPrinters]) => (
                                <div key={branchName} className="space-y-1">
                                  <p className="text-xs font-bold text-primary/70">{branchName}</p>
                                  <div className="flex flex-wrap gap-2">
                ...
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                  ...
                  {editProductForm && (
                    <form onSubmit={handleUpdateProduct} className="space-y-4">
                      ...
                      {/* اختيار الطابعات - مجمّعة حسب الفرع */}
                      <div className="space-y-2">
                        <Label className="text-foreground">{t('وجهة الطباعة (الطابعات)')}</Label>
                        {(() => {
                          const grouped = {};
